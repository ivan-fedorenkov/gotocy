 ...
 */
(function(a){if(typeof define==="function"&&define.amd){define(["jquery"],a)}else{if(typeof module==="object"&&module.exports){module.exports=a(require("jquery"))}else{a(window.jQuery)}}}(function(H){H.fn.fileinputLocales={};var h,Y,j,m,X,G,p,V,x,Z,z,o,D,E,k,O,c,b,B,s,w,T,F,d,I,ac,v,ad,S,A,q,ab,l,L,Q,K,P,g,i,C,J,a,R,U,aa,n,W,r,y,M,N,u,t,e,f;h=".fileinput";Y=window.URL||window.webkitURL;j=function(ae,ag,af){return ae!==undefined&&(af?ae===ag:ae.match(ag))};m=function(ae){if(navigator.appName!=="Microsoft Internet Explorer"){return false}if(ae===10){return new RegExp("msie\\s"+ae,"i").test(navigator.userAgent)}var ag=document.createElement("div"),af;ag.innerHTML="<!--[if IE "+ae+"]> <i></i> <![endif]-->";af=ag.getElementsByTagName("i").length;document.body.appendChild(ag);ag.parentNode.removeChild(ag);return af};X=function(){return new RegExp("Edge/[0-9]+","i").test(navigator.userAgent)};G=function(af,ah,ai,ae){var ag=ae?ah:ah.split(" ").join(h+" ")+h;af.off(ag).on(ag,ai)};p={data:{},init:function(af){var ae=af.initialPreview,ag=af.id;if(ae.length>0&&!y(ae)){ae=ae.split(af.initialPreviewDelimiter)}p.data[ag]={content:ae,config:af.initialPreviewConfig,tags:af.initialPreviewThumbTags,delimiter:af.initialPreviewDelimiter,template:af.previewGenericTemplate,msg:function(ah){return af._getMsgSelected(ah)},initId:af.previewInitId,footer:af._getLayoutTemplate("footer").replace(/\{progress}/g,af._renderThumbProgress()),isDelete:af.initialPreviewShowDelete,caption:af.initialCaption,actions:function(ah,al,ak,ai,aj){return af._renderFileActions(ah,al,ak,ai,aj)}}},fetch:function(ae){return p.data[ae].content.filter(function(af){return af!==null})},count:function(af,ae){return !!p.data[af]&&!!p.data[af].content?(ae?p.data[af].content.length:p.fetch(af).length):0},get:function(af,ak,am){var ag="init_"+ak,aj=p.data[af],ah=aj.config[ak],al=aj.initId+"-"+ag,ai,ae,an=" file-preview-initial";am=am===undefined?true:am;if(aj.content[ak]===null){return""}if(!r(ah)&&!r(ah.frameClass)){an+=" "+ah.frameClass}ai=aj.template.replace(/\{previewId}/g,al).replace(/\{frameClass}/g,an).replace(/\{fileindex}/g,ag).replace(/\{content}/g,aj.content[ak]).replace(/\{footer}/g,p.footer(af,ak,am));if(aj.tags.length&&aj.tags[ak]){ai=e(ai,aj.tags[ak])}if(!r(ah)&&!r(ah.frameAttr)){ae=H(document.createElement("div")).html(ai);ae.find(".file-preview-initial").attr(ah.frameAttr);ai=ae.html();ae.remove()}return ai},add:function(ak,ai,ah,ag,ae){var aj=H.extend(true,{},p.data[ak]),af;if(!y(ai)){ai=ai.split(aj.delimiter)}if(ae){af=aj.content.push(ai)-1;aj.config[af]=ah;aj.tags[af]=ag}else{af=ai.length;aj.content=ai;aj.config=ah;aj.tags=ag}p.data[ak]=aj;return af},set:function(al,aj,ah,ag,ae){var ak=H.extend(true,{},p.data[al]),ai,af;if(!aj||!aj.length){return}if(!y(aj)){aj=aj.split(ak.delimiter)}af=aj.filter(function(am){return am!==null});if(!af.length){return}if(ak.content===undefined){ak.content=[]}if(ak.config===undefined){ak.config=[]}if(ak.tags===undefined){ak.tags=[]}if(ae){for(ai=0;ai<aj.length;ai++){if(aj[ai]){ak.content.push(aj[ai])}}for(ai=0;ai<ah.length;ai++){if(ah[ai]){ak.config.push(ah[ai])}}for(ai=0;ai<ag.length;ai++){if(ag[ai]){ak.tags.push(ag[ai])}}}else{ak.content=aj;ak.config=ah;
ak.tags=ag}p.data[al]=ak},unset:function(ag,af){var ae=p.count(ag);if(!ae){return}if(ae===1){p.data[ag].content=[];p.data[ag].config=[];return}p.data[ag].content[af]=null;p.data[ag].config[af]=null},out:function(aj){var ah="",ai=p.data[aj],af,ae=p.count(aj,true);if(ae===0){return{content:"",caption:""}}for(var ag=0;ag<ae;ag++){ah+=p.get(aj,ag)}af=ai.msg(p.count(aj));return{content:ah,caption:af}},footer:function(af,al,ao){var ak=p.data[af];ao=ao===undefined?true:ao;if(ak.config.length===0||r(ak.config[al])){return""}var ah=ak.config[al],ap=M("caption",ah)?ah.caption:"",ag=M("width",ah)?ah.width:"auto",ae=M("url",ah)?ah.url:false,an=M("key",ah)?ah.key:null,aj=(ae===false)&&ao,ai=ak.isDelete?ak.actions(false,true,aj,ae,an):"",am=ak.footer.replace(/\{actions}/g,ai);return am.replace(/\{caption}/g,ap).replace(/\{width}/g,ag).replace(/\{indicator}/g,"").replace(/\{indicatorTitle}/g,"")}};V=function(ae,af){af=af||0;if(typeof ae==="number"){return ae}if(typeof ae==="string"){ae=parseFloat(ae)}return isNaN(ae)?af:ae};x=function(){return !!(window.File&&window.FileReader)};Z=function(){var ae=document.createElement("div");return !m(9)&&!X()&&(ae.draggable!==undefined||(ae.ondragstart!==undefined&&ae.ondrop!==undefined))};z=function(){return x()&&window.FormData};o=function(af,ae){af.removeClass(ae).addClass(ae)};D='style="width:{width};height:{height};"';E='      <param name="controller" value="true" />\n      <param name="allowFullScreen" value="true" />\n      <param name="allowScriptAccess" value="always" />\n      <param name="autoPlay" value="false" />\n      <param name="autoStart" value="false" />\n      <param name="quality" value="high" />\n';k='<div class="file-preview-other">\n   <span class="{previewFileIconClass}">{previewFileIcon}</span>\n</div>';O={removeIcon:'<i class="glyphicon glyphicon-trash text-danger"></i>',removeClass:"btn btn-xs btn-default",removeTitle:"Remove file",uploadIcon:'<i class="glyphicon glyphicon-upload text-info"></i>',uploadClass:"btn btn-xs btn-default",uploadTitle:"Upload file",indicatorNew:'<i class="glyphicon glyphicon-hand-down text-warning"></i>',indicatorSuccess:'<i class="glyphicon glyphicon-ok-sign text-success"></i>',indicatorError:'<i class="glyphicon glyphicon-exclamation-sign text-danger"></i>',indicatorLoading:'<i class="glyphicon glyphicon-hand-up text-muted"></i>',indicatorNewTitle:"Not uploaded yet",indicatorSuccessTitle:"Uploaded",indicatorErrorTitle:"Upload Error",indicatorLoadingTitle:"Uploading ..."};c='{preview}\n<div class="kv-upload-progress hide"></div>\n<div class="input-group {class}">\n   {caption}\n   <div class="input-group-btn">\n       {remove}\n       {cancel}\n       {upload}\n       {browse}\n   </div>\n</div>';b='{preview}\n<div class="kv-upload-progress hide"></div>\n{remove}\n{cancel}\n{upload}\n{browse}\n';B='<div class="file-preview {class}">\n    {close}    <div class="{dropClass}">\n    <div class="file-preview-thumbnails">\n    </div>\n    <div class="clearfix"></div>    <div class="file-preview-status text-center text-success"></div>\n    <div class="kv-fileinput-error"></div>\n    </div>\n</div>';w='<div class="close fileinput-remove">&times;</div>\n';s='<span class="glyphicon glyphicon-file kv-caption-icon"></span>';T='<div tabindex="500" class="form-control file-caption {class}">\n   <div class="file-caption-name"></div>\n</div>\n';F='<button type="{type}" tabindex="500" title="{title}" class="{css}" {status}>{icon}{label}</button>';d='<a href="{href}" tabindex="500" title="{title}" class="{css}" {status}>{icon}{label}</a>';I='<div tabindex="500" class="{css}" {status}>{icon}{label}</div>';ac='<div id="{id}" class="file-preview-detail-modal modal fade" tabindex="-1">\n  <div class="modal-dialog modal-lg">\n    <div class="modal-content">\n      <div class="modal-header">\n        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>\n        <h3 class="modal-title">{heading} <small>{title}</small></h3>\n      </div>\n      <div class="modal-body">\n           <pre>{body}</pre>\n      </div>\n    </div>\n  </div>\n</div>';
v='<div class="progress">\n    <div class="{class}" role="progressbar" aria-valuenow="{percent}" aria-valuemin="0" aria-valuemax="100" style="width:{percent}%;">\n        {percent}%\n     </div>\n</div>';ad='<div class="file-thumbnail-footer">\n    <div class="file-footer-caption" title="{caption}">{caption}</div>\n    {progress} {actions}\n</div>';S='<div class="file-actions">\n    <div class="file-footer-buttons">\n        {upload}{delete}{other}    </div>\n    <div class="file-upload-indicator" title="{indicatorTitle}">{indicator}</div>\n    <div class="clearfix"></div>\n</div>';A='<button type="button" class="kv-file-remove {removeClass}" title="{removeTitle}" {dataUrl}{dataKey}>{removeIcon}</button>\n';q='<button type="button" class="kv-file-upload {uploadClass}" title="{uploadTitle}">   {uploadIcon}\n</button>\n';ab='<button type="button" class="btn btn-default btn-xs btn-block" title="{zoomTitle}: {caption}" onclick="{dialog}">\n   {zoomInd}\n</button>\n';l='<div class="file-preview-frame{frameClass}" id="{previewId}" data-fileindex="{fileindex}">\n   {content}\n   {footer}\n</div>\n';L='<div class="file-preview-frame{frameClass}" id="{previewId}" data-fileindex="{fileindex}">\n    <object class="file-object" data="{data}" type="{type}" width="{width}" height="{height}">\n       '+k+"\n    </object>\n   {footer}\n</div>";Q='<div class="file-preview-frame{frameClass}" id="{previewId}" data-fileindex="{fileindex}">\n   <img src="{data}" class="file-preview-image" title="{caption}" alt="{caption}" '+D+">\n   {footer}\n</div>\n";K='<div class="file-preview-frame{frameClass}" id="{previewId}" data-fileindex="{fileindex}">\n   <pre class="file-preview-text" title="{caption}" '+D+">{data}</pre>\n   {zoom}\n   {footer}\n</div>";P='<div class="file-preview-frame{frameClass}" id="{previewId}" data-fileindex="{fileindex}" title="{caption}" '+D+'>\n   <video width="{width}" height="{height}" controls>\n       <source src="{data}" type="{type}">\n       '+k+"\n   </video>\n   {footer}\n</div>\n";g='<div class="file-preview-frame{frameClass}" id="{previewId}" data-fileindex="{fileindex}" title="{caption}" '+D+'>\n   <audio controls>\n       <source src="{data}" type="{type}">\n       '+k+"\n   </audio>\n   {footer}\n</div>";i='<div class="file-preview-frame{frameClass}" id="{previewId}" data-fileindex="{fileindex}" title="{caption}" '+D+'>\n   <object class="file-object" type="application/x-shockwave-flash" width="{width}" height="{height}" data="{data}">\n'+E+"       "+k+"\n   </object>\n   {footer}\n</div>\n";C='<div class="file-preview-frame{frameClass}" id="{previewId}" data-fileindex="{fileindex}" title="{caption}" '+D+'>\n   <object class="file-object" data="{data}" type="{type}" width="{width}" height="{height}">\n       <param name="movie" value="{caption}" />\n'+E+"         "+k+"\n   </object>\n   {footer}\n</div>";J='<div class="file-preview-frame{frameClass}" id="{previewId}" data-fileindex="{fileindex}" title="{caption}" '+D+'>\n   <div class="file-preview-other-frame">\n   '+k+'\n   </div>\n   <div class="file-preview-other-footer">{footer}</div>\n</div>';a={main1:c,main2:b,preview:B,close:w,zoom:ab,icon:s,caption:T,modal:ac,progress:v,footer:ad,actions:S,actionDelete:A,actionUpload:q,btnDefault:F,btnLink:d,btnBrowse:I};R={generic:l,html:L,image:Q,text:K,video:P,audio:g,flash:i,object:C,other:J};U=["image","html","text","video","audio","flash","object"];aa={image:{width:"auto",height:"160px"},html:{width:"213px",height:"160px"},text:{width:"160px",height:"136px"},video:{width:"213px",height:"160px"},audio:{width:"213px",height:"80px"},flash:{width:"213px",height:"160px"},object:{width:"160px",height:"160px"},other:{width:"160px",height:"160px"}};W={image:function(af,ae){return j(af,"image.*")||j(ae,/\.(gif|png|jpe?g)$/i)},html:function(af,ae){return j(af,"text/html")||j(ae,/\.(htm|html)$/i)},text:function(af,ae){return j(af,"text.*")||j(af,/\.(xml|javascript)$/i)||j(ae,/\.(txt|md|csv|nfo|ini|json|php|js|css)$/i)},video:function(af,ae){return j(af,"video.*")&&(j(af,/(ogg|mp4|mp?g|webm|3gp)$/i)||j(ae,/\.(og?|mp4|webm|mp?g|3gp)$/i))
},audio:function(af,ae){return j(af,"audio.*")&&(j(af,/(ogg|mp3|mp?g|wav)$/i)||j(ae,/\.(og?|mp3|mp?g|wav)$/i))},flash:function(af,ae){return j(af,"application/x-shockwave-flash",true)||j(ae,/\.(swf)$/i)},object:function(af,ae){return j(af,"application/pdf",true)||j(ae,/\.(pdf)$/i)},other:function(){return true}};r=function(af,ae){return af===undefined||af===null||af.length===0||(ae&&H.trim(af)==="")};y=function(ae){return Array.isArray(ae)||Object.prototype.toString.call(ae)==="[object Array]"};M=function(af,ae){return(typeof ae==="object"&&af in ae)};N=function(ae,ag,af){return(r(ae)||r(ae[ag]))?af:H(ae[ag])};u=function(){return Math.round(new Date().getTime()+(Math.random()*100))};t=function(ae){return ae.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&apos;")};e=function(ag,af){var ae=ag;if(!af){return ae}H.each(af,function(ah,ai){if(typeof ai==="function"){ai=ai()}ae=ae.split(ah).join(ai)});return ae};f=function(af){var ae=af.is("img")?af.attr("src"):af.find("source").attr("src");Y.revokeObjectURL(ae)};n=function(ag,af){var ae=this;ae.$element=H(ag);if(!ae._validate()){return}ae.isPreviewable=x();ae.isIE9=m(9);ae.isIE10=m(10);if(ae.isPreviewable||ae.isIE9){ae._init(af);ae._listen()}else{ae.$element.removeClass("file-loading")}};n.prototype={constructor:n,_init:function(af){var ae=this,ah=ae.$element,ag;H.each(af,function(ai,aj){switch(ai){case"minFileCount":case"maxFileCount":case"maxFileSize":ae[ai]=V(aj);break;default:ae[ai]=aj;break}});ae.fileInputCleared=false;ae.fileBatchCompleted=true;if(!ae.isPreviewable){ae.showPreview=false}ae.uploadFileAttr=!r(ah.attr("name"))?ah.attr("name"):"file_data";ae.reader=null;ae.formdata={};ae.clearStack();ae.uploadCount=0;ae.uploadStatus={};ae.uploadLog=[];ae.uploadAsyncCount=0;ae.loadedImages=[];ae.totalImagesCount=0;ae.ajaxRequests=[];ae.isError=false;ae.ajaxAborted=false;ae.cancelling=false;ag=ae._getLayoutTemplate("progress");ae.progressTemplate=ag.replace("{class}",ae.progressClass);ae.progressCompleteTemplate=ag.replace("{class}",ae.progressCompleteClass);ae.progressErrorTemplate=ag.replace("{class}",ae.progressErrorClass);ae.dropZoneEnabled=Z()&&ae.dropZoneEnabled;ae.isDisabled=ae.$element.attr("disabled")||ae.$element.attr("readonly");ae.isUploadable=z()&&!r(ae.uploadUrl);ae.slug=typeof af.slugCallback==="function"?af.slugCallback:ae._slugDefault;ae.mainTemplate=ae.showCaption?ae._getLayoutTemplate("main1"):ae._getLayoutTemplate("main2");ae.captionTemplate=ae._getLayoutTemplate("caption");ae.previewGenericTemplate=ae._getPreviewTemplate("generic");if(ae.resizeImage&&(ae.maxImageWidth||ae.maxImageHeight)){ae.imageCanvas=document.createElement("canvas");ae.imageCanvasContext=ae.imageCanvas.getContext("2d")}if(r(ae.$element.attr("id"))){ae.$element.attr("id",u())}if(ae.$container===undefined){ae.$container=ae._createContainer()}else{ae._refreshContainer()}ae.$dropZone=ae.$container.find(".file-drop-zone");ae.$progress=ae.$container.find(".kv-upload-progress");ae.$btnUpload=ae.$container.find(".fileinput-upload");ae.$captionContainer=N(af,"elCaptionContainer",ae.$container.find(".file-caption"));ae.$caption=N(af,"elCaptionText",ae.$container.find(".file-caption-name"));ae.$previewContainer=N(af,"elPreviewContainer",ae.$container.find(".file-preview"));ae.$preview=N(af,"elPreviewImage",ae.$container.find(".file-preview-thumbnails"));ae.$previewStatus=N(af,"elPreviewStatus",ae.$container.find(".file-preview-status"));ae.$errorContainer=N(af,"elErrorContainer",ae.$previewContainer.find(".kv-fileinput-error"));if(!r(ae.msgErrorClass)){o(ae.$errorContainer,ae.msgErrorClass)}ae.$errorContainer.hide();ae.fileActionSettings=H.extend(true,O,af.fileActionSettings);ae.previewInitId="preview-"+u();ae.id=ae.$element.attr("id");p.init(ae);ae._initPreview(true);ae._initPreviewDeletes();ae.options=af;ae._setFileDropZoneTitle();ae.$element.removeClass("file-loading");if(ae.$element.attr("disabled")){ae.disable()}},_validate:function(){var af=this,ae;
if(af.$element.attr("type")==="file"){return true}ae='<div class="help-block alert alert-warning"><h4>Invalid Input Type</h4>You must set an input <code>type = file</code> for <b>bootstrap-fileinput</b> plugin to initialize.</div>';af.$element.after(ae);return false},_errorsExist:function(){var ae=this,af;if(ae.$errorContainer.find("li").length){return true}af=H(document.createElement("div")).html(ae.$errorContainer.html());af.find("span.kv-error-close").remove();af.find("ul").remove();return H.trim(af.text()).length?true:false},_errorHandler:function(ae,ag){var af=this,ah=ae.target.error;if(ah.code===ah.NOT_FOUND_ERR){af._showError(af.msgFileNotFound.replace("{name}",ag))}else{if(ah.code===ah.SECURITY_ERR){af._showError(af.msgFileSecured.replace("{name}",ag))}else{if(ah.code===ah.NOT_READABLE_ERR){af._showError(af.msgFileNotReadable.replace("{name}",ag))}else{if(ah.code===ah.ABORT_ERR){af._showError(af.msgFilePreviewAborted.replace("{name}",ag))}else{af._showError(af.msgFilePreviewError.replace("{name}",ag))}}}}},_addError:function(ag){var af=this,ae=af.$errorContainer;if(ag&&ae.length){ae.html(af.errorCloseButton+ag);G(ae.find(".kv-error-close"),"click",function(){ae.fadeOut("slow")})}},_resetErrors:function(ag){var af=this,ae=af.$errorContainer;af.isError=false;af.$container.removeClass("has-error");ae.html("");if(ag){ae.fadeOut("slow")}else{ae.hide()}},_showFolderError:function(ag){var af=this,ae=af.$errorContainer,ah;if(!ag){return}ah=af.msgFoldersNotAllowed.replace(/\{n}/g,ag);af._addError(ah);o(af.$container,"has-error");ae.fadeIn(800);af._raise("filefoldererror",[ag,ah])},_showUploadError:function(ak,aj,ah){var af=this,ae=af.$errorContainer,ag=ah||"fileuploaderror",ai=aj&&aj.id?'<li data-file-id="'+aj.id+'">'+ak+"</li>":"<li>"+ak+"</li>";if(ae.find("ul").length===0){af._addError("<ul>"+ai+"</ul>")}else{ae.find("ul").append(ai)}ae.fadeIn(800);af._raise(ag,[aj,ak]);af.$container.removeClass("file-input-new");o(af.$container,"has-error");return true},_showError:function(aj,ai,ah){var af=this,ae=af.$errorContainer,ag=ah||"fileerror";ai=ai||{};ai.reader=af.reader;af._addError(aj);ae.fadeIn(800);af._raise(ag,[ai,aj]);if(!af.isUploadable){af._clearFileInput()}af.$container.removeClass("file-input-new");o(af.$container,"has-error");af.$btnUpload.attr("disabled",true);return true},_noFilesError:function(ai){var af=this,ag=af.minFileCount>1?af.filePlural:af.fileSingle,ah=af.msgFilesTooLess.replace("{n}",af.minFileCount).replace("{files}",ag),ae=af.$errorContainer;af._addError(ah);af.isError=true;af._updateFileDetails(0);ae.fadeIn(800);af._raise("fileerror",[ai,ah]);af._clearFileInput();o(af.$container,"has-error")},_parseError:function(ah,ai,ak){var af=this,ag=H.trim(ai+""),ae=ag.slice(-1)==="."?"":".",aj=ah.responseJSON!==undefined&&ah.responseJSON.error!==undefined?ah.responseJSON.error:ah.responseText;if(af.cancelling&&af.msgUploadAborted){ag=af.msgUploadAborted}if(af.showAjaxErrorDetails&&aj){aj=H.trim(aj.replace(/\n\s*\n/g,"\n"));aj=aj.length>0?"<pre>"+aj+"</pre>":"";ag+=ae+aj}else{ag+=ae}af.cancelling=false;return ak?"<b>"+ak+": </b>"+ag:ag},_parseFileType:function(ah){var af=this,aj,ai,ae,ag;for(ag=0;ag<U.length;ag+=1){ae=U[ag];aj=M(ae,af.fileTypeSettings)?af.fileTypeSettings[ae]:W[ae];ai=aj(ah.type,ah.name)?ae:"";if(!r(ai)){return ai}}return"other"},_parseFilePreviewIcon:function(ag,ai){var ae=this,af,ah=ae.previewFileIcon;if(ai&&ai.indexOf(".")>-1){af=ai.split(".").pop();if(ae.previewFileIconSettings&&ae.previewFileIconSettings[af]){ah=ae.previewFileIconSettings[af]}if(ae.previewFileExtSettings){H.each(ae.previewFileExtSettings,function(aj,ak){if(ae.previewFileIconSettings[aj]&&ak(af)){ah=ae.previewFileIconSettings[aj]}})}}if(ag.indexOf("{previewFileIcon}")>-1){return ag.replace(/\{previewFileIconClass}/g,ae.previewFileIconClass).replace(/\{previewFileIcon}/g,ah)}return ag},_raise:function(af,ah){var ae=this,ag=H.Event(af);if(ah!==undefined){ae.$element.trigger(ag,ah)}else{ae.$element.trigger(ag)}if(ag.isDefaultPrevented()){return false
}if(!ag.result){return ag.result}switch(af){case"filebatchuploadcomplete":case"filebatchuploadsuccess":case"fileuploaded":case"fileclear":case"filecleared":case"filereset":case"fileerror":case"filefoldererror":case"fileuploaderror":case"filebatchuploaderror":case"filedeleteerror":case"filecustomerror":case"filesuccessremove":break;default:ae.ajaxAborted=ag.result;break}return true},_listen:function(){var af=this,ag=af.$element,ae=ag.closest("form"),ah=af.$container;G(ag,"change",H.proxy(af._change,af));G(af.$btnFile,"click",H.proxy(af._browse,af));G(ae,"reset",H.proxy(af.reset,af));G(ah.find(".fileinput-remove:not([disabled])"),"click",H.proxy(af.clear,af));G(ah.find(".fileinput-cancel"),"click",H.proxy(af.cancel,af));af._initDragDrop();if(!af.isUploadable){G(ae,"submit",H.proxy(af._submitForm,af))}G(af.$container.find(".fileinput-upload"),"click",H.proxy(af._uploadClick,af))},_initDragDrop:function(){var ae=this,af=ae.$dropZone;if(ae.isUploadable&&ae.dropZoneEnabled&&ae.showPreview){G(af,"dragenter dragover",H.proxy(ae._zoneDragEnter,ae));G(af,"dragleave",H.proxy(ae._zoneDragLeave,ae));G(af,"drop",H.proxy(ae._zoneDrop,ae));G(H(document),"dragenter dragover drop",ae._zoneDragDropInit)}},_zoneDragDropInit:function(ae){ae.stopPropagation();ae.preventDefault()},_zoneDragEnter:function(ag){var ae=this,af=H.inArray("Files",ag.originalEvent.dataTransfer.types)>-1;ae._zoneDragDropInit(ag);if(ae.isDisabled||!af){ag.originalEvent.dataTransfer.effectAllowed="none";ag.originalEvent.dataTransfer.dropEffect="none";return}o(ae.$dropZone,"file-highlighted")},_zoneDragLeave:function(af){var ae=this;ae._zoneDragDropInit(af);if(ae.isDisabled){return}ae.$dropZone.removeClass("file-highlighted")},_zoneDrop:function(af){var ae=this;af.preventDefault();if(ae.isDisabled||r(af.originalEvent.dataTransfer.files)){return}ae._change(af,"dragdrop");ae.$dropZone.removeClass("file-highlighted")},_uploadClick:function(ai){var af=this,ah=af.$container.find(".fileinput-upload"),ae,ag=!ah.hasClass("disabled")&&r(ah.attr("disabled"));if(ai&&ai.isDefaultPrevented()){return}if(!af.isUploadable){if(ag&&ah.attr("type")!=="submit"){ae=ah.closest("form");if(ae.length){ae.trigger("submit")}ai.preventDefault()}return}ai.preventDefault();if(ag){af.upload()}},_submitForm:function(){var ae=this,af=ae.$element,ag=af.get(0).files;if(ag&&ae.minFileCount>0&&ae._getFileCount(ag.length)<ae.minFileCount){ae._noFilesError({});return false}return !ae._abort({})},_clearPreview:function(){var ae=this,af=!ae.showUploadedThumbs?ae.$preview.find(".file-preview-frame"):ae.$preview.find(".file-preview-frame:not(.file-preview-success)");af.remove();if(!ae.$preview.find(".file-preview-frame").length||!ae.showPreview){ae._resetUpload()}ae._validateDefaultPreview()},_initPreview:function(ah){var ae=this,ag=ae.initialCaption||"",af;if(!p.count(ae.id)){ae._clearPreview();if(ah){ae._setCaption(ag)}else{ae._initCaption()}return}af=p.out(ae.id);ag=ah&&ae.initialCaption?ae.initialCaption:af.caption;ae.$preview.html(af.content);ae._setCaption(ag);if(!r(af.content)){ae.$container.removeClass("file-input-new")}},_initPreviewDeletes:function(){var af=this,ae=af.deleteExtraData||{},ag=function(){var ah=af.isUploadable?p.count(af.id):af.$element.get(0).files.length;if(af.$preview.find(".kv-file-remove").length===0&&!ah){af.reset();af.initialCaption=""}};af.$preview.find(".kv-file-remove").each(function(){var aq=H(this),ao=aq.data("url")||af.deleteUrl,ai=aq.data("key");if(r(ao)||ai===undefined){return}var aj=aq.closest(".file-preview-frame"),ah=p.data[af.id],am,an,ap=aj.data("fileindex"),ak,al;ap=parseInt(ap.replace("init_",""));ak=r(ah.config)&&r(ah.config[ap])?null:ah.config[ap];al=r(ak)||r(ak.extra)?ae:ak.extra;if(typeof al==="function"){al=al()}an={id:aq.attr("id"),key:ai,extra:al};am=H.extend(true,{},{url:ao,type:"POST",dataType:"json",data:H.extend(true,{},{key:ai},al),beforeSend:function(ar){af.ajaxAborted=false;af._raise("filepredelete",[ai,ar,al]);if(af.ajaxAborted){ar.abort()}else{o(aj,"file-uploading");
o(aq,"disabled")}},success:function(au,aw,at){var av,ar;if(r(au)||r(au.error)){p.unset(af.id,ap);av=p.count(af.id);ar=av>0?af._getMsgSelected(av):"";af._raise("filedeleted",[ai,at,al]);af._setCaption(ar)}else{an.jqXHR=at;an.response=au;af._showError(au.error,an,"filedeleteerror");aj.removeClass("file-uploading");aq.removeClass("disabled");ag();return}aj.removeClass("file-uploading").addClass("file-deleted");aj.fadeOut("slow",function(){af._clearObjects(aj);aj.remove();ag();if(!av&&af.getFileStack().length===0){af._setCaption("");af.reset()}})},error:function(at,av,au){var ar=af._parseError(at,au);an.jqXHR=at;an.response={};af._showError(ar,an,"filedeleteerror");aj.removeClass("file-uploading");ag()}},af.ajaxDeleteSettings);G(aq,"click",function(){if(!af._validateMinCount()){return false}H.ajax(am)})})},_clearObjects:function(ae){ae.find("video audio").each(function(){this.pause();H(this).remove()});ae.find("img object div").each(function(){H(this).remove()})},_clearFileInput:function(){var ag=this,ah=ag.$element,ai,ae,af;if(r(ah.val())){return}if(ag.isIE9||ag.isIE10){ai=ah.closest("form");ae=H(document.createElement("form"));af=H(document.createElement("div"));ah.before(af);if(ai.length){ai.after(ae)}else{af.after(ae)}ae.append(ah).trigger("reset");af.before(ah).remove();ae.remove()}else{ah.val("")}ag.fileInputCleared=true},_resetUpload:function(){var ae=this;ae.uploadCache={content:[],config:[],tags:[],append:true};ae.uploadCount=0;ae.uploadStatus={};ae.uploadLog=[];ae.uploadAsyncCount=0;ae.loadedImages=[];ae.totalImagesCount=0;ae.$btnUpload.removeAttr("disabled");ae._setProgress(0);o(ae.$progress,"hide");ae._resetErrors(false);ae.ajaxAborted=false;ae.ajaxRequests=[];ae._resetCanvas()},_resetCanvas:function(){var ae=this;if(ae.canvas&&ae.imageCanvasContext){ae.imageCanvasContext.clearRect(0,0,ae.canvas.width,ae.canvas.height)}},_hasInitialPreview:function(){var ae=this;return !ae.overwriteInitial&&p.count(ae.id)},_resetPreview:function(){var ae=this,af,ag;if(p.count(ae.id)){af=p.out(ae.id);ae.$preview.html(af.content);ag=ae.initialCaption?ae.initialCaption:af.caption;ae._setCaption(ag)}else{ae._clearPreview();ae._initCaption()}},_clearDefaultPreview:function(){var ae=this;ae.$preview.find(".file-default-preview").remove()},_validateDefaultPreview:function(){var ae=this;if(!ae.showPreview||r(ae.defaultPreviewContent)){return}ae.$preview.html('<div class="file-default-preview">'+ae.defaultPreviewContent+"</div>");ae.$container.removeClass("file-input-new")},_resetPreviewThumbs:function(ag){var ae=this,af;if(ag){ae._clearPreview();ae.clearStack();return}if(ae._hasInitialPreview()){af=p.out(ae.id);ae.$preview.html(af.content);ae._setCaption(af.caption);ae._initPreviewDeletes()}else{ae._clearPreview()}},_getLayoutTemplate:function(af){var ae=this,ag=M(af,ae.layoutTemplates)?ae.layoutTemplates[af]:a[af];if(r(ae.customLayoutTags)){return ag}return e(ag,ae.customLayoutTags)},_getPreviewTemplate:function(af){var ae=this,ag=M(af,ae.previewTemplates)?ae.previewTemplates[af]:R[af];if(r(ae.customPreviewTags)){return ag}return e(ag,ae.customPreviewTags)},_getOutData:function(ag,af,ah){var ae=this;ag=ag||{};af=af||{};ah=ah||ae.filestack.slice(0)||{};return{form:ae.formdata,files:ah,filenames:ae.filenames,extra:ae._getExtraData(),response:af,reader:ae.reader,jqXHR:ag}},_getMsgSelected:function(ag){var ae=this,af=ag===1?ae.fileSingle:ae.filePlural;return ae.msgSelected.replace("{n}",ag).replace("{files}",af)},_getThumbs:function(ae){ae=ae||"";return this.$preview.find(".file-preview-frame:not(.file-preview-initial)"+ae)},_getExtraData:function(ae,ag){var af=this,ah=af.uploadExtraData;if(typeof af.uploadExtraData==="function"){ah=af.uploadExtraData(ae,ag)}return ah},_initXhr:function(ag,ae,ah){var af=this;if(ag.upload){ag.upload.addEventListener("progress",function(ak){var al=0,ai=ak.loaded||ak.position,aj=ak.total;if(ak.lengthComputable){al=Math.ceil(ai/aj*100)}if(ae){af._setAsyncUploadStatus(ae,al,ah)}else{af._setProgress(Math.ceil(al))}},false)
}return ag},_ajaxSubmit:function(ai,ak,al,ag,ae,ah){var af=this,aj;af._raise("filepreajax",[ae,ah]);af._uploadExtra(ae,ah);aj=H.extend(true,{},{xhr:function(){var am=H.ajaxSettings.xhr();return af._initXhr(am,ae,af.getFileStack().length)},url:af.uploadUrl,type:"POST",dataType:"json",data:af.formdata,cache:false,processData:false,contentType:false,beforeSend:ai,success:ak,complete:al,error:ag},af.ajaxSettings);af.ajaxRequests.push(H.ajax(aj))},_initUploadSuccess:function(ah,ao,ae){var am=this,af,aj,ak,ap,al,ag,an,ai;if(!am.showPreview||typeof ah!=="object"||H.isEmptyObject(ah)){return}if(ah.initialPreview!==undefined&&ah.initialPreview.length>0){am.hasInitData=true;al=ah.initialPreview||[];ag=ah.initialPreviewConfig||[];an=ah.initialPreviewThumbTags||[];af=ah.append===undefined||ah.append?true:false;am.overwriteInitial=false;if(ao!==undefined){if(!ae){ak=p.add(am.id,al,ag[0],an[0],af);aj=p.get(am.id,ak,false);ap=H(aj).hide();ao.after(ap).fadeOut("slow",function(){ap.fadeIn("slow").css("display:inline-block");am._initPreviewDeletes();am._clearFileInput();ao.remove()})}else{ai=ao.attr("data-fileindex");am.uploadCache.content[ai]=al[0];am.uploadCache.config[ai]=ag[0];am.uploadCache.tags[ai]=an[0];am.uploadCache.append=af}}else{p.set(am.id,al,ag,an,af);am._initPreview();am._initPreviewDeletes()}}},_initSuccessThumbs:function(){var ae=this;if(!ae.showPreview){return}ae._getThumbs(".file-preview-success").each(function(){var ag=H(this),af=ag.find(".kv-file-remove");af.removeAttr("disabled");G(af,"click",function(){var ah=ae._raise("filesuccessremove",[ag.attr("id"),ag.data("fileindex")]);f(ag);if(ah===false){return}ag.fadeOut("slow",function(){ag.remove();if(!ae.$preview.find(".file-preview-frame").length){ae.reset()}})})})},_checkAsyncComplete:function(){var af=this,ae,ag;for(ag=0;ag<af.filestack.length;ag++){if(af.filestack[ag]){ae=af.previewInitId+"-"+ag;if(H.inArray(ae,af.uploadLog)===-1){return false}}}return(af.uploadAsyncCount===af.uploadLog.length)},_uploadExtra:function(ae,ag){var af=this,ah=af._getExtraData(ae,ag);if(ah.length===0){return}H.each(ah,function(ai,aj){af.formdata.append(ai,aj)})},_uploadSingle:function(ar,ai,ah){var ap=this,ax=ap.getFileStack().length,ao=new FormData(),aw,ae=ap.previewInitId+"-"+ar,al,aq,an,av,ag=ap.filestack.length>0||!H.isEmptyObject(ap.uploadExtraData),aj,am,af,au,ak,at={id:ae,index:ar};ap.formdata=ao;if(ap.showPreview){al=H("#"+ae+":not(.file-preview-initial)");an=al.find(".kv-file-upload");av=al.find(".kv-file-remove");H("#"+ae).find(".file-thumb-progress").removeClass("hide")}if(ax===0||!ag||(an&&an.hasClass("disabled"))||ap._abort(at)){return}ak=function(az,ay){ap.updateStack(az,undefined);ap.uploadLog.push(ay);if(ap._checkAsyncComplete()){ap.fileBatchCompleted=true}};aq=function(){if(!ap.fileBatchCompleted){return}setTimeout(function(){if(ap.showPreview){p.set(ap.id,ap.uploadCache.content,ap.uploadCache.config,ap.uploadCache.tags,ap.uploadCache.append);if(ap.hasInitData){ap._initPreview();ap._initPreviewDeletes()}}ap.unlock();ap._clearFileInput();ap._raise("filebatchuploadcomplete",[ap.filestack,ap._getExtraData()]);ap.uploadCount=0;ap.uploadStatus={};ap.uploadLog=[];ap._setProgress(100)},100)};aj=function(ay){aw=ap._getOutData(ay);ap.fileBatchCompleted=false;if(ap.showPreview){if(!al.hasClass("file-preview-success")){ap._setThumbStatus(al,"Loading");o(al,"file-uploading")}an.attr("disabled",true);av.attr("disabled",true)}if(!ah){ap.lock()}ap._raise("filepreupload",[aw,ae,ar]);H.extend(true,at,aw);if(ap._abort(at)){ay.abort();ap._setProgressCancelled()}};am=function(az,aA,ay){aw=ap._getOutData(ay,az);H.extend(true,at,aw);setTimeout(function(){if(r(az)||r(az.error)){if(ap.showPreview){ap._setThumbStatus(al,"Success");an.hide();ap._initUploadSuccess(az,al,ah)}ap._raise("fileuploaded",[aw,ae,ar]);if(!ah){ap.updateStack(ar,undefined)}else{ak(ar,ae)}}else{ap._showUploadError(az.error,at);ap._setPreviewError(al,ar);if(ah){ak(ar,ae)}}},100)};af=function(){setTimeout(function(){if(ap.showPreview){an.removeAttr("disabled");
av.removeAttr("disabled");al.removeClass("file-uploading")}if(!ah){ap.unlock(false);ap._clearFileInput()}else{aq()}ap._initSuccessThumbs()},100)};au=function(az,aB,aA){var ay=ap._parseError(az,aA,(ah?ai[ar].name:null));setTimeout(function(){if(ah){ak(ar,ae)}ap.uploadStatus[ae]=100;ap._setPreviewError(al,ar);H.extend(true,at,ap._getOutData(az));ap._showUploadError(ay,at)},100)};ao.append(ap.uploadFileAttr,ai[ar],ap.filenames[ar]);ao.append("file_id",ar);ap._ajaxSubmit(aj,am,af,au,ae,ar)},_uploadBatch:function(){var an=this,ae=an.filestack,al=ae.length,ai={},aj,am,ak,ah,af=an.filestack.length>0||!H.isEmptyObject(an.uploadExtraData),ag;an.formdata=new FormData();if(al===0||!af||an._abort(ai)){return}ag=function(){H.each(ae,function(ao){an.updateStack(ao,undefined)});an._clearFileInput()};aj=function(ao){an.lock();var ap=an._getOutData(ao);if(an.showPreview){an._getThumbs().each(function(){var at=H(this),aq=at.find(".kv-file-upload"),ar=at.find(".kv-file-remove");if(!at.hasClass("file-preview-success")){an._setThumbStatus(at,"Loading");o(at,"file-uploading")}aq.attr("disabled",true);ar.attr("disabled",true)})}an._raise("filebatchpreupload",[ap]);if(an._abort(ap)){ao.abort();an._setProgressCancelled()}};am=function(at,av,ar){var au=an._getOutData(ar,at),ap=an._getThumbs(),ao=0,aq=r(at)||r(at.errorkeys)?[]:at.errorkeys;if(r(at)||r(at.error)){an._raise("filebatchuploadsuccess",[au]);ag();if(an.showPreview){ap.each(function(){var ax=H(this),aw=ax.find(".kv-file-upload");ax.find(".kv-file-upload").hide();an._setThumbStatus(ax,"Success");ax.removeClass("file-uploading");aw.removeAttr("disabled")});an._initUploadSuccess(at)}else{an.reset()}}else{if(an.showPreview){ap.each(function(){var ay=H(this),ax=ay.find(".kv-file-remove"),aw=ay.find(".kv-file-upload");ay.removeClass("file-uploading");aw.removeAttr("disabled");ax.removeAttr("disabled");if(aq.length===0){an._setPreviewError(ay);return}if(H.inArray(ao,aq)!==-1){an._setPreviewError(ay)}else{ay.find(".kv-file-upload").hide();an._setThumbStatus(ay,"Success");an.updateStack(ao,undefined)}ao++});an._initUploadSuccess(at)}an._showUploadError(at.error,au,"filebatchuploaderror")}};ah=function(){an._setProgress(100);an.unlock();an._initSuccessThumbs();an._clearFileInput();an._raise("filebatchuploadcomplete",[an.filestack,an._getExtraData()])};ak=function(ap,at,ar){var aq=an._getOutData(ap),ao=an._parseError(ap,ar);an._showUploadError(ao,aq,"filebatchuploaderror");an.uploadFileCount=al-1;if(!an.showPreview){return}an._getThumbs().each(function(){var av=H(this),au=av.attr("data-fileindex");av.removeClass("file-uploading");if(an.filestack[au]!==undefined){an._setPreviewError(av)}});an._getThumbs().removeClass("file-uploading");an._getThumbs(" .kv-file-upload").removeAttr("disabled");an._getThumbs(" .kv-file-delete").removeAttr("disabled")};H.each(ae,function(ao,ap){if(!r(ae[ao])){an.formdata.append(an.uploadFileAttr,ap,an.filenames[ao])}});an._ajaxSubmit(aj,am,ah,ak)},_uploadExtraOnly:function(){var af=this,ai={},ag,ah,aj,ae;af.formdata=new FormData();if(af._abort(ai)){return}ag=function(ak){af.lock();var al=af._getOutData(ak);af._raise("filebatchpreupload",[al]);af._setProgress(50);ai.data=al;ai.xhr=ak;if(af._abort(ai)){ak.abort();af._setProgressCancelled()}};ah=function(al,an,ak){var am=af._getOutData(ak,al);if(r(al)||r(al.error)){af._raise("filebatchuploadsuccess",[am]);af._clearFileInput();af._initUploadSuccess(al)}else{af._showUploadError(al.error,am,"filebatchuploaderror")}};aj=function(){af._setProgress(100);af.unlock();af._clearFileInput();af._raise("filebatchuploadcomplete",[af.filestack,af._getExtraData()])};ae=function(al,ao,an){var am=af._getOutData(al),ak=af._parseError(al,an);ai.data=am;af._showUploadError(ak,am,"filebatchuploaderror")};af._ajaxSubmit(ag,ah,aj,ae)},_initFileActions:function(){var ae=this;if(!ae.showPreview){return}ae.$preview.find(".kv-file-remove").each(function(){var ai=H(this),aj=ai.closest(".file-preview-frame"),af,am=aj.attr("id"),ak=aj.attr("data-fileindex"),al,ah,ag;
G(ai,"click",function(){ag=ae._raise("filepreremove",[am,ak]);if(ag===false||!ae._validateMinCount()){return false}af=aj.hasClass("file-preview-error");f(aj);aj.fadeOut("slow",function(){ae.updateStack(ak,undefined);ae._clearObjects(aj);aj.remove();if(am&&af){ae.$errorContainer.find('li[data-file-id="'+am+'"]').fadeOut("fast",function(){H(this).remove();if(!ae._errorsExist()){ae._resetErrors()}})}var ap=ae.getFileStack(true),an=ap.length,ao=p.count(ae.id),aq=ae.showPreview&&ae.$preview.find(".file-preview-frame").length;ae._clearFileInput();if(an===0&&ao===0&&!aq){ae.reset()}else{al=ao+an;ah=al>1?ae._getMsgSelected(al):(ap[0]?ae._getFileNames()[0]:"");ae._setCaption(ah)}ae._raise("fileremoved",[am,ak])})})});ae.$preview.find(".kv-file-upload").each(function(){var af=H(this);G(af,"click",function(){var ag=af.closest(".file-preview-frame"),ah=ag.attr("data-fileindex");if(!ag.hasClass("file-preview-error")){ae._uploadSingle(ah,ae.filestack,false)}})})},_hideFileIcon:function(){if(this.overwriteInitial){this.$captionContainer.find(".kv-caption-icon").hide()}},_showFileIcon:function(){this.$captionContainer.find(".kv-caption-icon").show()},_previewDefault:function(ai,ak,am){if(!this.showPreview){return}var ao=this,an="",af=ai?ai.name:"",aj=Y.createObjectURL(ai),ag=ak.slice(ak.lastIndexOf("-")+1),ae=ao.previewSettings.other||aa.other,al=ao._renderFileFooter(ai.name,ae.width),ah=ao._parseFilePreviewIcon(ao._getPreviewTemplate("other"),af);if(am===true){if(!ao.isUploadable){al+='<div class="file-other-error" title="'+ao.fileActionSettings.indicatorErrorTitle+'">'+ao.fileActionSettings.indicatorError+"</div>"}}ao._clearDefaultPreview();ao.$preview.append("\n"+ah.replace(/\{previewId}/g,ak).replace(/\{frameClass}/g,an).replace(/\{fileindex}/g,ag).replace(/\{caption}/g,ao.slug(ai.name)).replace(/\{width}/g,ae.width).replace(/\{height}/g,ae.height).replace(/\{type}/g,ai.type).replace(/\{data}/g,aj).replace(/\{footer}/g,al));if(am===true&&ao.isUploadable){ao._setThumbStatus(H("#"+ak),"Error")}},_previewFile:function(ar,av,ap,ag,az){if(!this.showPreview){return}var al=this,ai=al._parseFileType(av),ax=av?av.name:"",au=al.slug(ax),aq,aw,ak=al.allowedPreviewTypes,af=al.allowedPreviewMimeTypes,ae=al._getPreviewTemplate(ai),am=ak&&ak.indexOf(ai)>=0,ao,ay=M(ai,al.previewSettings)?al.previewSettings[ai]:aa[ai],ah=af&&af.indexOf(av.type)!==-1,an=al._renderFileFooter(au,ay.width),at="",aj=ag.slice(ag.lastIndexOf("-")+1);if(am||ah){ae=al._parseFilePreviewIcon(ae,ax.split(".").pop());if(ai==="text"){aw=t(ap.target.result);ao="text-"+u();aq=ae.replace(/\{zoom}/g,al._getLayoutTemplate("zoom"));at=al._getLayoutTemplate("modal").replace("{id}",ao).replace(/\{title}/g,au).replace(/\{body}/g,aw).replace(/\{heading}/g,al.msgZoomModalHeading);aq=aq.replace(/\{previewId}/g,ag).replace(/\{caption}/g,au).replace(/\{width}/g,ay.width).replace(/\{height}/g,ay.height).replace(/\{frameClass}/g,"").replace(/\{zoomInd}/g,al.zoomIndicator).replace(/\{footer}/g,an).replace(/\{fileindex}/g,aj).replace(/\{type}/g,av.type).replace(/\{zoomTitle}/g,al.msgZoomTitle).replace(/\{dialog}/g,"$('#"+ao+"').modal('show')").replace(/\{data}/g,aw)+at}else{aq=ae.replace(/\{previewId}/g,ag).replace(/\{caption}/g,au).replace(/\{frameClass}/g,"").replace(/\{type}/g,av.type).replace(/\{fileindex}/g,aj).replace(/\{width}/g,ay.width).replace(/\{height}/g,ay.height).replace(/\{footer}/g,an).replace(/\{data}/g,az)}al._clearDefaultPreview();al.$preview.append("\n"+aq);al._validateImage(ar,ag,au,av.type)}else{al._previewDefault(av,ag)}},_slugDefault:function(ae){return r(ae)?"":String(ae).replace(/[\-\[\]\/\{}:;#%=\(\)\*\+\?\\\^\$\|<>&"']/g,"_")},_readFiles:function(ae){this.reader=new FileReader();var ar=this,at=ar.$element,ak=ar.$preview,am=ar.reader,ap=ar.$previewContainer,aq=ar.$previewStatus,an=ar.msgLoading,ag=ar.msgProgress,al=ar.previewInitId,af=ae.length,aj=ar.fileTypeSettings,ah=ar.filestack.length,ao,ai=function(az,aw,au,av){var ay=H.extend(true,{},ar._getOutData({},{},ae),{id:au,index:av}),ax={id:au,index:av,file:aw,files:ae};
ar._previewDefault(aw,au,true);if(ar.isUploadable){ar.addToStack(undefined)}setTimeout(ao(av+1),100);ar._initFileActions();if(ar.removeFromPreviewOnError){H("#"+au).remove()}return ar.isUploadable?ar._showUploadError(az,ay):ar._showError(az,ax)};ar.loadedImages=[];ar.totalImagesCount=0;H.each(ae,function(av,au){var aw=ar.fileTypeSettings.image||W.image;if(aw&&aw(au.type)){ar.totalImagesCount++}});ao=function(aH){if(r(at.attr("multiple"))){af=1}if(aH>=af){if(ar.isUploadable&&ar.filestack.length>0){ar._raise("filebatchselected",[ar.getFileStack()])}else{ar._raise("filebatchselected",[ae])}ap.removeClass("file-thumb-loading");aq.html("");return}var aE=ah+aH,av=al+"-"+aE,aD,aK=ae[aH],aI=ar.slug(aK.name),aC=(aK.size||0)/1000,ay,aw="",aA=Y.createObjectURL(aK),aL=0,aG,az,aM,aJ,au=ar.allowedFileTypes,aF=r(au)?"":au.join(", "),aB=ar.allowedFileExtensions,ax=r(aB)?"":aB.join(", ");if(!r(aB)){aw=new RegExp("\\.("+aB.join("|")+")$","i")}aC=aC.toFixed(2);if(ar.maxFileSize>0&&aC>ar.maxFileSize){az=ar.msgSizeTooLarge.replace("{name}",aI).replace("{size}",aC).replace("{maxSize}",ar.maxFileSize);ar.isError=ai(az,aK,av,aH);return}if(!r(au)&&y(au)){for(aG=0;aG<au.length;aG+=1){aM=au[aG];ay=aj[aM];aJ=(ay!==undefined&&ay(aK.type,aI));aL+=r(aJ)?0:aJ.length}if(aL===0){az=ar.msgInvalidFileType.replace("{name}",aI).replace("{types}",aF);ar.isError=ai(az,aK,av,aH);return}}if(aL===0&&!r(aB)&&y(aB)&&!r(aw)){aJ=j(aI,aw);aL+=r(aJ)?0:aJ.length;if(aL===0){az=ar.msgInvalidFileExtension.replace("{name}",aI).replace("{extensions}",ax);ar.isError=ai(az,aK,av,aH);return}}if(!ar.showPreview){ar.addToStack(aK);setTimeout(ao(aH+1),100);ar._raise("fileloaded",[aK,av,aH,am]);return}if(ak.length>0&&FileReader!==undefined){aq.html(an.replace("{index}",aH+1).replace("{files}",af));ap.addClass("file-thumb-loading");am.onerror=function(aN){ar._errorHandler(aN,aI)};am.onload=function(aN){ar._previewFile(aH,aK,aN,av,aA);ar._initFileActions()};am.onloadend=function(){az=ag.replace("{index}",aH+1).replace("{files}",af).replace("{percent}",50).replace("{name}",aI);setTimeout(function(){aq.html(az);ar._updateFileDetails(af);ao(aH+1)},100);ar._raise("fileloaded",[aK,av,aH,am])};am.onprogress=function(aP){if(aP.lengthComputable){var aO=(aP.loaded/aP.total)*100,aN=Math.ceil(aO);az=ag.replace("{index}",aH+1).replace("{files}",af).replace("{percent}",aN).replace("{name}",aI);setTimeout(function(){aq.html(az)},100)}};aD=M("text",aj)?aj.text:W.text;if(aD(aK.type,aI)){am.readAsText(aK,ar.textEncoding)}else{am.readAsArrayBuffer(aK)}}else{ar._previewDefault(aK,av);setTimeout(function(){ao(aH+1);ar._updateFileDetails(af)},100);ar._raise("fileloaded",[aK,av,aH,am])}ar.addToStack(aK)};ao(0);ar._updateFileDetails(af,false)},_updateFileDetails:function(af){var al=this,am=al.$element,ai=al.getFileStack(),ae=(am[0].files[0]&&am[0].files[0].name)||(ai.length&&ai[0].name)||"",ak=al.slug(ae),ag=al.isUploadable?ai.length:af,aj=p.count(al.id)+ag,ah=ag>1?al._getMsgSelected(aj):ak;if(al.isError){al.$previewContainer.removeClass("file-thumb-loading");al.$previewStatus.html("");al.$captionContainer.find(".kv-caption-icon").hide()}else{al._showFileIcon()}al._setCaption(ah,al.isError);al.$container.removeClass("file-input-new file-input-ajax-new");if(arguments.length===1){al._raise("fileselect",[af,ak])}if(p.count(al.id)){al._initPreviewDeletes()}},_setThumbStatus:function(al,ae){var af=this;if(!af.showPreview){return}var ai="indicator"+ae,ak=ai+"Title",ah="file-preview-"+ae.toLowerCase(),aj=al.find(".file-upload-indicator"),ag=af.fileActionSettings;al.removeClass("file-preview-success file-preview-error file-preview-loading");if(ae==="Error"){al.find(".kv-file-upload").attr("disabled",true)}aj.html(ag[ai]);aj.attr("title",ag[ak]);al.addClass(ah)},_setProgressCancelled:function(){var ae=this;ae._setProgress(100,ae.$progress,ae.msgCancelled)},_setProgress:function(aj,ag,af){var ae=this,ai=Math.min(aj,100),ah=ai<100?ae.progressTemplate:(af?ae.progressErrorTemplate:ae.progressCompleteTemplate);ag=ag||ae.$progress;
if(!r(ah)){ag.html(ah.replace(/\{percent}/g,ai));if(af){ag.find('[role="progressbar"]').html(af)}}},_setFileDropZoneTitle:function(){var ae=this,af=ae.$container.find(".file-drop-zone");af.find("."+ae.dropZoneTitleClass).remove();if(!ae.isUploadable||!ae.showPreview||af.length===0||ae.getFileStack().length>0||!ae.dropZoneEnabled){return}if(af.find(".file-preview-frame").length===0&&r(ae.defaultPreviewContent)){af.prepend('<div class="'+ae.dropZoneTitleClass+'">'+ae.dropZoneTitle+"</div>")}ae.$container.removeClass("file-input-new");o(ae.$container,"file-input-ajax-new")},_setAsyncUploadStatus:function(ae,ai,ah){var af=this,ag=0;af._setProgress(ai,H("#"+ae).find(".file-thumb-progress"));af.uploadStatus[ae]=ai;H.each(af.uploadStatus,function(aj,ak){ag+=ak});af._setProgress(Math.ceil(ag/ah))},_validateMinCount:function(){var af=this,ae=af.isUploadable?af.getFileStack().length:af.$element.get(0).files.length;if(af.validateInitialCount&&af.minFileCount>0&&af._getFileCount(ae-1)<af.minFileCount){af._noFilesError({});return false}return true},_getFileCount:function(ag){var ae=this,af=0;if(ae.validateInitialCount&&!ae.overwriteInitial){af=p.count(ae.id);ag+=af}return ag},_getFileName:function(ae){return ae&&ae.name?this.slug(ae.name):undefined},_getFileNames:function(ae){var af=this;return af.filenames.filter(function(ag){return(ae?ag!==undefined:ag!==undefined&&ag!==null)})},_setPreviewError:function(ah,af,ag){var ae=this;if(af){ae.updateStack(af,ag)}if(ae.removeFromPreviewOnError){ah.remove()}else{ae._setThumbStatus(ah,"Error")}},_checkDimensions:function(ak,an,ae,ar,ah,am,aj){var ap=this,ag,al,aq=an==="Small"?"min":"max",ai=ap[aq+"Image"+am],af,ao;if(r(ai)||!ae.length){return}af=ae[0];al=(am==="Width")?af.naturalWidth||af.width:af.naturalHeight||af.height;ao=an==="Small"?al>=ai:al<=ai;if(ao){return}ag=ap["msgImage"+am+an].replace("{name}",ah).replace("{size}",ai);ap._showUploadError(ag,aj);ap._setPreviewError(ar,ak,null)},_validateImage:function(aj,al,af,am){var an=this,ah=an.$preview,ag,ak,ai,ao=ah.find("#"+al),ae=ao.find("img");af=af||"Untitled";if(!ae.length){return}G(ae,"load",function(){ak=ao.width();ai=ah.width();if(ak>ai){ae.css("width","100%");ao.css("width","97%")}ag={ind:aj,id:al};an._checkDimensions(aj,"Small",ae,ao,af,"Width",ag);an._checkDimensions(aj,"Small",ae,ao,af,"Height",ag);if(!an.resizeImage){an._checkDimensions(aj,"Large",ae,ao,af,"Width",ag);an._checkDimensions(aj,"Large",ae,ao,af,"Height",ag)}an._raise("fileimageloaded",[al]);an.loadedImages.push({ind:aj,img:ae,thumb:ao,pid:al,typ:am});an._validateAllImages();Y.revokeObjectURL(ae.attr("src"))})},_validateAllImages:function(){var al=this,ai,ag,ae,am,aj,af,ah={},ak;if(al.loadedImages.length!==al.totalImagesCount){return}al._raise("fileimagesloaded");if(!al.resizeImage){return}ak=al.isUploadable?al._showUploadError:al._showError;for(ai=0;ai<al.loadedImages.length;ai++){ag=al.loadedImages[ai];ae=ag.img;am=ag.thumb;aj=ag.pid;af=ag.ind;ah={id:aj,index:af};if(!al._getResizedImage(ae[0],ag.typ,aj,af)){ak(al.msgImageResizeError,ah,"fileimageresizeerror");al._setPreviewError(am,af)}}al._raise("fileimagesresized")},_getResizedImage:function(ak,ap,an,ah){var au=this,af=ak.naturalWidth,at=ak.naturalHeight,ao=1,ar=au.maxImageWidth||af,aq=au.maxImageHeight||at,am=(af&&at),aj,ai,ag=au.imageCanvas,ae=au.imageCanvasContext;if(!am){return false}if(af===ar&&at===aq){return true}ap=ap||au.resizeDefaultImageType;aj=af>ar;ai=at>aq;if(au.resizePreference==="width"){ao=aj?ar/af:(ai?aq/at:1)}else{ao=ai?aq/at:(aj?ar/af:1)}au._resetCanvas();af*=ao;at*=ao;ag.width=af;ag.height=at;try{ae.drawImage(ak,0,0,af,at);ag.toBlob(function(av){au._raise("fileimageresized",[an,ah]);au.filestack[ah]=av},ap,au.resizeQuality);return true}catch(al){return false}},_initBrowse:function(af){var ae=this;ae.$btnFile=af.find(".btn-file");ae.$btnFile.append(ae.$element)},_initCaption:function(){var ae=this,af=ae.initialCaption||"";if(ae.overwriteInitial||r(af)){ae.$caption.html("");return false}ae._setCaption(af);
return true},_setCaption:function(aj,ai){var af=this,ak,ag,al,ah,ae=af.getFileStack();if(!af.$caption.length){return}if(ai){ak=H("<div>"+af.msgValidationError+"</div>").text();al=ae.length;if(al){ah=al===1&&ae[0]?af._getFileNames()[0]:af._getMsgSelected(al)}else{ah=af._getMsgSelected(af.msgNo)}ag='<span class="'+af.msgValidationErrorClass+'">'+af.msgValidationErrorIcon+(r(aj)?ah:aj)+"</span>"}else{if(r(aj)){return}ak=H("<div>"+aj+"</div>").text();ag=af._getLayoutTemplate("icon")+ak}af.$caption.html(ag);af.$caption.attr("title",ak);af.$captionContainer.find(".file-caption-ellipsis").attr("title",ak)},_createContainer:function(){var ae=this,af=H(document.createElement("div")).attr({"class":"file-input file-input-new"}).html(ae._renderMain());ae.$element.before(af);ae._initBrowse(af);return af},_refreshContainer:function(){var ae=this,af=ae.$container;af.before(ae.$element);af.html(ae._renderMain());ae._initBrowse(af)},_renderMain:function(){var af=this,ah=(af.isUploadable&&af.dropZoneEnabled)?" file-drop-zone":"file-drop-disabled",aj=!af.showClose?"":af._getLayoutTemplate("close"),ai=!af.showPreview?"":af._getLayoutTemplate("preview").replace(/\{class}/g,af.previewClass).replace(/\{dropClass}/g,ah),ag=af.isDisabled?af.captionClass+" file-caption-disabled":af.captionClass,ae=af.captionTemplate.replace(/\{class}/g,ag+" kv-fileinput-caption");return af.mainTemplate.replace(/\{class}/g,af.mainClass).replace(/\{preview}/g,ai).replace(/\{close}/g,aj).replace(/\{caption}/g,ae).replace(/\{upload}/g,af._renderButton("upload")).replace(/\{remove}/g,af._renderButton("remove")).replace(/\{cancel}/g,af._renderButton("cancel")).replace(/\{browse}/g,af._renderButton("browse"))},_renderButton:function(ai){var am=this,ae=am._getLayoutTemplate("btnDefault"),ah=am[ai+"Class"],ak=am[ai+"Title"],aj=am[ai+"Icon"],al=am[ai+"Label"],ag=am.isDisabled?" disabled":"",af="button";switch(ai){case"remove":if(!am.showRemove){return""}break;case"cancel":if(!am.showCancel){return""}ah+=" hide";break;case"upload":if(!am.showUpload){return""}if(am.isUploadable&&!am.isDisabled){ae=am._getLayoutTemplate("btnLink").replace("{href}",am.uploadUrl)}else{af="submit"}break;case"browse":ae=am._getLayoutTemplate("btnBrowse");break;default:return""}ah+=ai==="browse"?" btn-file":" fileinput-"+ai+" fileinput-"+ai+"-button";if(!r(al)){al=' <span class="'+am.buttonLabelClass+'">'+al+"</span>"}return ae.replace("{type}",af).replace("{css}",ah).replace("{title}",ak).replace("{status}",ag).replace("{icon}",aj).replace("{label}",al)},_renderThumbProgress:function(){return'<div class="file-thumb-progress hide">'+this.progressTemplate.replace(/\{percent}/g,"0")+"</div>"},_renderFileFooter:function(af,aj){var ae=this,ah=ae.fileActionSettings,ak,ag,ai=ae._getLayoutTemplate("footer");if(ae.isUploadable){ak=ai.replace(/\{actions}/g,ae._renderFileActions(true,true,false,false,false));ag=ak.replace(/\{caption}/g,af).replace(/\{width}/g,aj).replace(/\{progress}/g,ae._renderThumbProgress()).replace(/\{indicator}/g,ah.indicatorNew).replace(/\{indicatorTitle}/g,ah.indicatorNewTitle)}else{ag=ai.replace(/\{actions}/g,"").replace(/\{caption}/g,af).replace(/\{progress}/g,"").replace(/\{width}/g,aj).replace(/\{indicator}/g,"").replace(/\{indicatorTitle}/g,"")}ag=e(ag,ae.previewThumbTags);return ag},_renderFileActions:function(ah,ae,al,ag,aq){if(!ah&&!ae){return""}var ar=this,ak=ag===false?"":' data-url="'+ag+'"',ai=aq===false?"":' data-key="'+aq+'"',am=ar._getLayoutTemplate("actionDelete"),an="",ap=ar._getLayoutTemplate("actions"),af=ar.otherActionButtons.replace(/\{dataKey}/g,ai),aj=ar.fileActionSettings,ao=al?aj.removeClass+" disabled":aj.removeClass;am=am.replace(/\{removeClass}/g,ao).replace(/\{removeIcon}/g,aj.removeIcon).replace(/\{removeTitle}/g,aj.removeTitle).replace(/\{dataUrl}/g,ak).replace(/\{dataKey}/g,ai);if(ah){an=ar._getLayoutTemplate("actionUpload").replace(/\{uploadClass}/g,aj.uploadClass).replace(/\{uploadIcon}/g,aj.uploadIcon).replace(/\{uploadTitle}/g,aj.uploadTitle)}return ap.replace(/\{delete}/g,am).replace(/\{upload}/g,an).replace(/\{other}/g,af)
},_browse:function(af){var ae=this;ae._raise("filebrowse");if(af&&af.isDefaultPrevented()){return}if(ae.isError&&!ae.isUploadable){ae.clear()}ae.$captionContainer.focus()},_change:function(at){var al=this,av=al.$element;if(!al.isUploadable&&r(av.val())&&al.fileInputCleared){al.fileInputCleared=false;return}al.fileInputCleared=false;var ak,af,aw,ai=arguments.length>1,ae=al.isUploadable,an=0,aq,am,ao,ag=ai?at.originalEvent.dataTransfer.files:av.get(0).files,ar=al.filestack.length,aj=r(av.attr("multiple")),ah=(aj&&ar>0),ap=0,au=function(ax,aA,ay,az){var aC=H.extend(true,{},al._getOutData({},{},ag),{id:ay,index:az}),aB={id:ay,index:az,file:aA,files:ag};return al.isUploadable?al._showUploadError(ax,aC):al._showError(ax,aB)};al.reader=null;al._resetUpload();al._hideFileIcon();if(al.isUploadable){al.$container.find(".file-drop-zone ."+al.dropZoneTitleClass).remove()}if(ai){ak=[];while(ag[an]){aq=ag[an];if(!aq.type&&aq.size%4096===0){ap++}else{ak.push(aq)}an++}}else{if(at.target.files===undefined){ak=at.target&&at.target.value?[{name:at.target.value.replace(/^.+\\/,"")}]:[]}else{ak=at.target.files}}if(r(ak)||ak.length===0){if(!ae){al.clear()}al._showFolderError(ap);al._raise("fileselectnone");return}al._resetErrors();ao=ak.length;aw=al._getFileCount(al.isUploadable?(al.getFileStack().length+ao):ao);if(al.maxFileCount>0&&aw>al.maxFileCount){if(!al.autoReplace||ao>al.maxFileCount){am=(al.autoReplace&&ao>al.maxFileCount)?ao:aw;af=al.msgFilesTooMany.replace("{m}",al.maxFileCount).replace("{n}",am);al.isError=au(af,null,null,null);al.$captionContainer.find(".kv-caption-icon").hide();al._setCaption("",true);al.$container.removeClass("file-input-new file-input-ajax-new");return}if(aw>al.maxFileCount){al._resetPreviewThumbs(ae)}}else{if(!ae||ah){al._resetPreviewThumbs(false);if(ah){al.clearStack()}}else{if(ae&&ar===0&&(!p.count(al.id)||al.overwriteInitial)){al._resetPreviewThumbs(true)}}}if(al.isPreviewable){al._readFiles(ak)}else{al._updateFileDetails(1)}al._showFolderError(ap)},_abort:function(ag){var ae=this,af;if(ae.ajaxAborted&&typeof ae.ajaxAborted==="object"&&ae.ajaxAborted.message!==undefined){af=H.extend(true,{},ae._getOutData(),ag);af.abortData=ae.ajaxAborted.data||{};af.abortMessage=ae.ajaxAborted.message;ae.cancel();ae._setProgress(100,ae.$progress,ae.msgCancelled);ae._showUploadError(ae.ajaxAborted.message,af,"filecustomerror");return true}return false},_resetFileStack:function(){var ae=this,ag=0,ah=[],af=[];ae._getThumbs().each(function(){var ak=H(this),aj=ak.attr("data-fileindex"),ai=ae.filestack[aj];if(aj===-1){return}if(ai!==undefined){ah[ag]=ai;af[ag]=ae._getFileName(ai);ak.attr({id:ae.previewInitId+"-"+ag,"data-fileindex":ag});ag++}else{ak.attr({id:"uploaded-"+u(),"data-fileindex":"-1"})}});ae.filestack=ah;ae.filenames=af},clearStack:function(){var ae=this;ae.filestack=[];ae.filenames=[];return ae.$element},updateStack:function(ag,af){var ae=this;ae.filestack[ag]=af;ae.filenames[ag]=ae._getFileName(af);return ae.$element},addToStack:function(af){var ae=this;ae.filestack.push(af);ae.filenames.push(ae._getFileName(af));return ae.$element},getFileStack:function(ae){var af=this;return af.filestack.filter(function(ag){return(ae?ag!==undefined:ag!==undefined&&ag!==null)})},lock:function(){var ae=this;ae._resetErrors();ae.disable();if(ae.showRemove){o(ae.$container.find(".fileinput-remove"),"hide")}if(ae.showCancel){ae.$container.find(".fileinput-cancel").removeClass("hide")}ae._raise("filelock",[ae.filestack,ae._getExtraData()]);return ae.$element},unlock:function(af){var ae=this;if(af===undefined){af=true}ae.enable();if(ae.showCancel){o(ae.$container.find(".fileinput-cancel"),"hide")}if(ae.showRemove){ae.$container.find(".fileinput-remove").removeClass("hide")}if(af){ae._resetFileStack()}ae._raise("fileunlock",[ae.filestack,ae._getExtraData()]);return ae.$element},cancel:function(){var af=this,ah=af.ajaxRequests,ae=ah.length,ag;if(ae>0){for(ag=0;ag<ae;ag+=1){af.cancelling=true;ah[ag].abort()}}af._setProgressCancelled();af._getThumbs().each(function(){var aj=H(this),ai=aj.attr("data-fileindex");
aj.removeClass("file-uploading");if(af.filestack[ai]!==undefined){aj.find(".kv-file-upload").removeClass("disabled").removeAttr("disabled");aj.find(".kv-file-remove").removeClass("disabled").removeAttr("disabled")}af.unlock()});return af.$element},clear:function(){var ae=this,af;ae.$btnUpload.removeAttr("disabled");ae._getThumbs().find("video,audio,img").each(function(){f(H(this))});ae._resetUpload();ae.clearStack();ae._clearFileInput();ae._resetErrors(true);ae._raise("fileclear");if(ae._hasInitialPreview()){ae._showFileIcon();ae._resetPreview();ae._initPreviewDeletes();ae.$container.removeClass("file-input-new")}else{ae._getThumbs().each(function(){ae._clearObjects(H(this))});if(ae.isUploadable){p.data[ae.id]={}}ae.$preview.html("");af=(!ae.overwriteInitial&&ae.initialCaption.length>0)?ae.initialCaption:"";ae._setCaption(af);ae.$caption.attr("title","");o(ae.$container,"file-input-new");ae._validateDefaultPreview()}if(ae.$container.find(".file-preview-frame").length===0){if(!ae._initCaption()){ae.$captionContainer.find(".kv-caption-icon").hide()}}ae._hideFileIcon();ae._raise("filecleared");ae.$captionContainer.focus();ae._setFileDropZoneTitle();return ae.$element},reset:function(){var ae=this;ae._resetPreview();ae.$container.find(".fileinput-filename").text("");ae._raise("filereset");o(ae.$container,"file-input-new");if(ae.$preview.find(".file-preview-frame").length||ae.isUploadable&&ae.dropZoneEnabled){ae.$container.removeClass("file-input-new")}ae._setFileDropZoneTitle();ae.clearStack();ae.formdata={};return ae.$element},disable:function(){var ae=this;ae.isDisabled=true;ae._raise("filedisabled");ae.$element.attr("disabled","disabled");ae.$container.find(".kv-fileinput-caption").addClass("file-caption-disabled");ae.$container.find(".btn-file, .fileinput-remove, .fileinput-upload, .file-preview-frame button").attr("disabled",true);ae._initDragDrop();return ae.$element},enable:function(){var ae=this;ae.isDisabled=false;ae._raise("fileenabled");ae.$element.removeAttr("disabled");ae.$container.find(".kv-fileinput-caption").removeClass("file-caption-disabled");ae.$container.find(".btn-file, .fileinput-remove, .fileinput-upload, .file-preview-frame button").removeAttr("disabled");ae._initDragDrop();return ae.$element},upload:function(){var ag=this,af=ag.getFileStack().length,ak={},ai,aj,ae,ah=!H.isEmptyObject(ag._getExtraData());if(ag.minFileCount>0&&ag._getFileCount(af)<ag.minFileCount){ag._noFilesError(ak);return}if(!ag.isUploadable||ag.isDisabled||(af===0&&!ah)){return}ag._resetUpload();ag.$progress.removeClass("hide");ag.uploadCount=0;ag.uploadStatus={};ag.uploadLog=[];ag.lock();ag._setProgress(2);if(af===0&&ah){ag._uploadExtraOnly();return}ae=ag.filestack.length;ag.hasInitData=false;if(ag.uploadAsync){aj=ag._getOutData();ag._raise("filebatchpreupload",[aj]);ag.fileBatchCompleted=false;ag.uploadCache={content:[],config:[],tags:[],append:true};ag.uploadAsyncCount=ag.getFileStack().length;for(ai=0;ai<ae;ai++){ag.uploadCache.content[ai]=null;ag.uploadCache.config[ai]=null;ag.uploadCache.tags[ai]=null}for(ai=0;ai<ae;ai++){if(ag.filestack[ai]!==undefined){ag._uploadSingle(ai,ag.filestack,true)}}return}ag._uploadBatch();return ag.$element},destroy:function(){var ae=this,af=ae.$container;af.find(".file-drop-zone").off();ae.$element.insertBefore(af).off(h).removeData();af.off().remove();return ae.$element},refresh:function(af){var ae=this,ag=ae.$element;af=af?H.extend(true,{},ae.options,af):ae.options;ae.destroy();ag.fileinput(af);if(ag.val()){ag.trigger("change.fileinput")}return ag}};H.fn.fileinput=function(ag){if(!x()&&!m(9)){return}var af=Array.apply(null,arguments),ae=[];af.shift();this.each(function(){var ah=H(this),ak=ah.data("fileinput"),ai=typeof ag==="object"&&ag,am=ai.language||ah.data("language")||"en",al={},aj;if(!ak){if(am!=="en"&&!r(H.fn.fileinputLocales[am])){al=H.fn.fileinputLocales[am]}aj=H.extend(true,{},H.fn.fileinput.defaults,H.fn.fileinputLocales.en,al,ai,ah.data());ak=new n(this,aj);ah.data("fileinput",ak)}if(typeof ag==="string"){ae.push(ak[ag].apply(ak,af))
}});switch(ae.length){case 0:return this;case 1:return ae[0];default:return ae}};H.fn.fileinput.defaults={language:"en",showCaption:true,showPreview:true,showRemove:true,showUpload:true,showCancel:true,showClose:true,showUploadedThumbs:true,autoReplace:false,mainClass:"",previewClass:"",captionClass:"",mainTemplate:null,initialCaption:"",initialPreview:[],initialPreviewDelimiter:"*$$*",initialPreviewConfig:[],initialPreviewThumbTags:[],previewThumbTags:{},initialPreviewShowDelete:true,removeFromPreviewOnError:false,deleteUrl:"",deleteExtraData:{},overwriteInitial:true,layoutTemplates:a,previewTemplates:R,allowedPreviewTypes:U,allowedPreviewMimeTypes:null,allowedFileTypes:null,allowedFileExtensions:null,defaultPreviewContent:null,customLayoutTags:{},customPreviewTags:{},previewSettings:aa,fileTypeSettings:W,previewFileIcon:'<i class="glyphicon glyphicon-file"></i>',previewFileIconClass:"file-icon-4x",previewFileIconSettings:{},previewFileExtSettings:{},buttonLabelClass:"hidden-xs",browseIcon:'<i class="glyphicon glyphicon-folder-open"></i>&nbsp;',browseClass:"btn btn-primary",removeIcon:'<i class="glyphicon glyphicon-trash"></i>',removeClass:"btn btn-default",cancelIcon:'<i class="glyphicon glyphicon-ban-circle"></i>',cancelClass:"btn btn-default",uploadIcon:'<i class="glyphicon glyphicon-upload"></i>',uploadClass:"btn btn-default",uploadUrl:null,uploadAsync:true,uploadExtraData:{},minImageWidth:null,minImageHeight:null,maxImageWidth:null,maxImageHeight:null,resizeImage:false,resizePreference:"width",resizeQuality:0.92,resizeDefaultImageType:"image/jpeg",maxFileSize:0,minFileCount:0,maxFileCount:0,validateInitialCount:false,msgValidationErrorClass:"text-danger",msgValidationErrorIcon:'<i class="glyphicon glyphicon-exclamation-sign"></i> ',msgErrorClass:"file-error-message",progressThumbClass:"progress-bar progress-bar-success progress-bar-striped active",progressClass:"progress-bar progress-bar-success progress-bar-striped active",progressCompleteClass:"progress-bar progress-bar-success",progressErrorClass:"progress-bar progress-bar-danger",previewFileType:"image",zoomIndicator:'<i class="glyphicon glyphicon-zoom-in"></i>',elCaptionContainer:null,elCaptionText:null,elPreviewContainer:null,elPreviewImage:null,elPreviewStatus:null,elErrorContainer:null,errorCloseButton:'<span class="close kv-error-close">&times;</span>',slugCallback:null,dropZoneEnabled:true,dropZoneTitleClass:"file-drop-zone-title",fileActionSettings:{},otherActionButtons:"",textEncoding:"UTF-8",ajaxSettings:{},ajaxDeleteSettings:{},showAjaxErrorDetails:true};H.fn.fileinputLocales.en={fileSingle:"file",filePlural:"files",browseLabel:"Browse &hellip;",removeLabel:"Remove",removeTitle:"Clear selected files",cancelLabel:"Cancel",cancelTitle:"Abort ongoing upload",uploadLabel:"Upload",uploadTitle:"Upload selected files",msgNo:"No",msgCancelled:"Cancelled",msgZoomTitle:"View details",msgZoomModalHeading:"Detailed Preview",msgSizeTooLarge:'File "{name}" (<b>{size} KB</b>) exceeds maximum allowed upload size of <b>{maxSize} KB</b>.',msgFilesTooLess:"You must select at least <b>{n}</b> {files} to upload.",msgFilesTooMany:"Number of files selected for upload <b>({n})</b> exceeds maximum allowed limit of <b>{m}</b>.",msgFileNotFound:'File "{name}" not found!',msgFileSecured:'Security restrictions prevent reading the file "{name}".',msgFileNotReadable:'File "{name}" is not readable.',msgFilePreviewAborted:'File preview aborted for "{name}".',msgFilePreviewError:'An error occurred while reading the file "{name}".',msgInvalidFileType:'Invalid type for file "{name}". Only "{types}" files are supported.',msgInvalidFileExtension:'Invalid extension for file "{name}". Only "{extensions}" files are supported.',msgUploadAborted:"The file upload was aborted",msgValidationError:"Validation Error",msgLoading:"Loading file {index} of {files} &hellip;",msgProgress:"Loading file {index} of {files} - {name} - {percent}% completed.",msgSelected:"{n} {files} selected",msgFoldersNotAllowed:"Drag & drop files only! {n} folder(s) dropped were skipped.",msgImageWidthSmall:'Width of image file "{name}" must be at least {size} px.',msgImageHeightSmall:'Height of image file "{name}" must be at least {size} px.',msgImageWidthLarge:'Width of image file "{name}" cannot exceed {size} px.',msgImageHeightLarge:'Height of image file "{name}" cannot exceed {size} px.',msgImageResizeError:"Could not get the image dimensions to resize.",msgImageResizeException:"Error while resizing the image.<pre>{errors}</pre>",dropZoneTitle:"Drag & drop files here &hellip;"};
H.fn.fileinput.Constructor=n;H(document).ready(function(){var ae=H("input.file[type=file]");if(ae.length){ae.fileinput()}})}));$(document).ready(function(g){equalHeight(".equal-height");g(".nav > li > ul li > ul").css("left",g(".nav > li > ul").width());var h=g(".nav > li");h.hover(function(){if(g("body").hasClass("navigation-fixed-bottom")){if(g(window).width()>768){var o=g(window).height()-(g(this).offset().top-g(window).scrollTop());if(o<g(this).children(".child-navigation").height()){g(this).children(".child-navigation").addClass("position-bottom")}else{g(this).children(".child-navigation").removeClass("position-bottom")}}}});setNavigationPosition();g(".tool-tip").tooltip();var l=g("select");if(l.length>0){l.selectpicker()}var n=g(".bootstrap-select");var c=g(".dropdown-menu");n.on("shown.bs.dropdown",function(){c.removeClass("animation-fade-out");c.addClass("animation-fade-in")});n.on("hide.bs.dropdown",function(){c.removeClass("animation-fade-in");c.addClass("animation-fade-out")});n.on("hidden.bs.dropdown",function(){var o=g(this);g(o).addClass("open");setTimeout(function(){g(o).removeClass("open")},100)});l.change(function(){if(g(this).val()!=""){g(".form-search .bootstrap-select.open").addClass("selected-option-check")}else{g(".form-search  .bootstrap-select.open").removeClass("selected-option-check")}});g("#form-contact-submit").bind("click",function(o){g("#form-contact").validate({submitHandler:function(){g.post("http://assets.gotocy.com/static/php/contact.php",g("#form-contact").serialize(),function(p){g("#form-status").html(p);g("#form-contact-submit").attr("disabled","true")});return false}})});if(g(".video").length>0){g(".video").fitVids()}var a=g("#price-input");if(a.length>0){a.slider({from:100,to:5000000,step:100,heterogeneity:["30/5000","60/500000"],format:{format:"###,###",locale:"en"},dimension:"&nbsp;€"})}g("#map .marker-style").css("opacity",".5 !important");g("#map .marker-style").css("bakground-color","red");g(window).scroll(function(){var o=g(window).scrollTop()/1.5;o=Math.round(o);if(g("body").hasClass("navigation-fixed-bottom")){if(g(window).scrollTop()>g(window).height()-g(".navigation").height()){g(".navigation").addClass("navigation-fix-to-top")}else{g(".navigation").removeClass("navigation-fix-to-top")}}if(g(window).width()>768){if(g("#map").hasClass("has-parallax")){g("#map .gm-style").css("margin-top",o+"px");g("#map .leaflet-map-pane").css("margin-top",o+"px")}if(g("#slider").hasClass("has-parallax")){g(".homepage-slider").css("top",o+"px")}}});g('.navigation .nav a[href^="#"], a[href^="#"].roll').on("click",function(q){q.preventDefault();var p=this.hash,o=g(p);if(g(window).width()>768){g("html, body").stop().animate({scrollTop:o.offset().top-g(".navigation").height()},2000)}else{g("html, body").stop().animate({scrollTop:o.offset().top},2000)}});var j=g(".rating-overall");if(j.length>0){j.raty({path:"http://assets.gotocy.com/static/img",readOnly:true,score:function(){return g(this).attr("data-score")}})}var k=g(".rating-individual");if(k.length>0){k.raty({path:"http://assets.gotocy.com/static/img",readOnly:true,score:function(){return g(this).attr("data-score")}})}var f=g(".rating-user");if(f.length>0){g(".rating-user .inner").raty({path:"http://assets.gotocy.com/static/img",starOff:"big-star-off.png",starOn:"big-star-on.png",width:150,targetType:"number",targetFormat:"Rating: {score}",click:function(p,o){showRatingForm()}})}g("#agent-switch").on("ifClicked",function(o){agentState()});g("#create-account-user").on("ifClicked",function(o){g("#agent-switch").data("agent-state","");agentState()});var m=g(".bookmark");if(m.data("bookmark-state")=="empty"){m.removeClass("bookmark-added")}else{if(m.data("bookmark-state")=="added"){m.addClass("bookmark-added")}}m.on("click",function(){if(m.data("bookmark-state")=="empty"){m.data("bookmark-state","added");m.addClass("bookmark-added")}else{if(m.data("bookmark-state")=="added"){m.data("bookmark-state","empty");m.removeClass("bookmark-added")}}});if(g("body").hasClass("navigation-fixed-bottom")){g("#page-content").css("padding-top",g(".navigation").height())}if(g(".property").hasClass("masonry")){var b=g(".masonry-grid");
b.imagesLoaded(function(){b.masonry({gutter:15,itemSelector:".masonry"})});if(g(window).width()>991){g(".masonry").hover(function(){g(".masonry").each(function(){g(".masonry").addClass("masonry-hide-other");g(this).removeClass("masonry-show")});g(this).addClass("masonry-show")},function(){g(".masonry").each(function(){g(".masonry").removeClass("masonry-hide-other")})});var d={after:"0s",enter:"bottom",move:"20px",over:".5s",easing:"ease-out",viewportFactor:0.33,reset:false,init:true};window.scrollReveal=new scrollReveal(d)}}var i=g("#pano-popup-container");if(i.length>0){i.magnificPopup({items:[{src:"#pano-popup",type:"inline"}],key:"pano",midClick:true,callbacks:{open:function(){initializePano(panoConfigUrl)}},closeBtnInside:false,closeOnBgClick:true,overflowY:"scroll"})}var e=g("#gallery-popup-container");if(e.length>0){e.magnificPopup({items:images.map(function(o){return{src:o}}),gallery:{enabled:true},type:"image",overflowY:"scroll"})}if(g(".checkbox").length>0){g("input").iCheck()}if(g(".radio").length>0){g("input").iCheck()}if(g(".submit-pricing").length>0){g(".btn").click(function(){g(".submit-pricing .buttons td").each(function(){g(this).removeClass("package-selected")});g(this).parent().css("opacity","1");g(this).parent().addClass("package-selected")})}if(g("ul.language-switch-list").length>0){g("ul.language-switch-list a").click(function(o){o.preventDefault();g(this).tab("show")})}if(g("form.form-submitted").length>0){setTimeout(function(){g("#submit-success-btn").css("display","none");g("#submit-btn").css("display","initial")},4000)}});$(window).on("resize",function(){setNavigationPosition();equalHeight(".equal-height")});$(window).load(function(){$(".search-box.map").addClass("show-search-box");showAllButton();var a=$(".number");if(a.length>0){a.waypoint(function(){initCounter()},{offset:"100%"})}agentState()});function initializePano(b){function a(){}pano=$("#pano");if(pano.length>0){pano.css("max-height","636px");pano.css("height",($(window).height()-40)+"px");pano.css("width","inherit");player=new pano2vrPlayer("pano");skin=new pano2vrSkin(player,"http://assets.gotocy.com/static/pano2vr/");player.readConfigUrl(b);setTimeout(function(){a()},10)}}function setNavigationPosition(){$(".nav > li").each(function(){if($(this).hasClass("has-child")){var a=$(this).children(".child-navigation").width()+$(this).children(".child-navigation").children("li").children(".child-navigation").width();if(($(this).children(".child-navigation").offset().left+a)>$(window).width()){$(this).children(".child-navigation").addClass("navigation-to-left")}}})}function agentState(){var c=$("#agency .form-group").height();var b=$("#agent-switch");var a=$("#agency");if(b.data("agent-state")=="is-agent"){b.iCheck("check");a.removeClass("disabled");a.addClass("enabled");b.data("agent-state","")}else{b.data("agent-state","is-agent");a.removeClass("enabled");a.addClass("disabled")}}function initCounter(){$(".number").countTo({speed:3000,refreshInterval:50})}function showAllButton(){var c=2;var a=$(".layout-expandable");var b=a.height();a.height($(".layout-expandable .row").height()*c-5);$(".show-all").on("click",function(){if(a.hasClass("layout-expanded")){a.height($(".layout-expandable .row").height()*c-5);a.removeClass("layout-expanded");$(".show-all").removeClass("layout-expanded")}else{a.height(b);a.addClass("layout-expanded");$(".show-all").addClass("layout-expanded")}})}function showRatingForm(){$(".rating-form").css("height",$(".rating-form form").height()+85+"px")}function equalHeight(b){var a=0,c=0,f=new Array(),e,d=0;$(b).each(function(){e=$(this);$(e).height("auto");topPostion=e.position().top;if(c!=topPostion){for(currentDiv=0;currentDiv<f.length;currentDiv++){f[currentDiv].height(a)}f.length=0;c=topPostion;a=e.height();f.push(e)}else{f.push(e);a=(a<e.height())?(e.height()):(a)}for(currentDiv=0;currentDiv<f.length;currentDiv++){f[currentDiv].height(a)}})};var Utils={};Utils.appendScript=function(b){var a=document.createElement("script");a.type="text/javascript";a.src=b;a.async=false;$(document.body).append(a)};Utils.getPath=function(a){return a};Utils.getPropertySearchPath=function(f,b,a,d){var e=f==="ru"?"ru":"default";var c=[];if(b){c.push(b)}if(a){c.push(a)}if(d){c.push(d)}return(f==="en"?"/":f==="ru"?"/ru/":"/el/")+Utils.propertySearchPath[c.join("_")][e]};Utils.propertySearchPath={"":{"default":"properties-in-cyprus",ru:"nedvizhimost-na-kipre"},AYIA_NAPA:{"default":"properties-in-ayia-napa",ru:"nedvizhimost-v-ayia-nape"},FAMAGUSTA:{"default":"properties-in-famagusta",ru:"nedvizhimost-v-famaguste"},LARNACA:{"default":"properties-in-larnaca",ru:"nedvizhimost-v-larnake"},LIMASSOL:{"default":"properties-in-limassol",ru:"nedvizhimost-v-limassole"},NICOSIA:{"default":"properties-in-nicosia",ru:"nedvizhimost-v-nikosii"},PAPHOS:{"default":"properties-in-paphos",ru:"nedvizhimost-v-pafose"},PROTARAS:{"default":"properties-in-protaras",ru:"nedvizhimost-v-protarase"},TROODOS:{"default":"properties-in-troodos",ru:"nedvizhimost-v-troodose"},LONG_TERM:{"default":"properties-long-term-rental-in-cyprus",ru:"arenda-nedvizhimosti-na-kipre"},AYIA_NAPA_LONG_TERM:{"default":"properties-long-term-rental-in-ayia-napa",ru:"arenda-nedvizhimosti-v-ayia-nape"},FAMAGUSTA_LONG_TERM:{"default":"properties-long-term-rental-in-famagusta",ru:"arenda-nedvizhimosti-v-famaguste"},LARNACA_LONG_TERM:{"default":"properties-long-term-rental-in-larnaca",ru:"arenda-nedvizhimosti-v-larnake"},LIMASSOL_LONG_TERM:{"default":"properties-long-term-rental-in-limassol",ru:"arenda-nedvizhimosti-v-limassole"},NICOSIA_LONG_TERM:{"default":"properties-long-term-rental-in-nicosia",ru:"arenda-nedvizhimosti-v-nikosii"},PAPHOS_LONG_TERM:{"default":"properties-long-term-rental-in-paphos",ru:"arenda-nedvizhimosti-v-pafose"},PROTARAS_LONG_TERM:{"default":"properties-long-term-rental-in-protaras",ru:"arenda-nedvizhimosti-v-protarase"},TROODOS_LONG_TERM:{"default":"properties-long-term-rental-in-troodos",ru:"arenda-nedvizhimosti-v-troodose"},SHORT_TERM:{"default":"properties-short-term-rental-in-cyprus",ru:"otdih-na-kipre-nedvizhimost"},AYIA_NAPA_SHORT_TERM:{"default":"properties-short-term-rental-in-ayia-napa",ru:"otdih-na-kipre-v-ayia-nape-nedvizhimost"},FAMAGUSTA_SHORT_TERM:{"default":"properties-short-term-rental-in-famagusta",ru:"otdih-na-kipre-v-famaguste-nedvizhimost"},LARNACA_SHORT_TERM:{"default":"properties-short-term-rental-in-larnaca",ru:"otdih-na-kipre-v-larnake-nedvizhimost"},LIMASSOL_SHORT_TERM:{"default":"properties-short-term-rental-in-limassol",ru:"otdih-na-kipre-v-limassole-nedvizhimost"},NICOSIA_SHORT_TERM:{"default":"properties-short-term-rental-in-nicosia",ru:"otdih-na-kipre-v-nikosii-nedvizhimost"},PAPHOS_SHORT_TERM:{"default":"properties-short-term-rental-in-paphos",ru:"otdih-na-kipre-v-pafose-nedvizhimost"},PROTARAS_SHORT_TERM:{"default":"properties-short-term-rental-in-protaras",ru:"otdih-na-kipre-v-protarase-nedvizhimost"},TROODOS_SHORT_TERM:{"default":"properties-short-term-rental-in-troodos",ru:"otdih-na-kipre-v-troodose-nedvizhimost"},SALE:{"default":"properties-for-sale-in-cyprus",ru:"prodazha-nedvizhimosti-na-kipre"},AYIA_NAPA_SALE:{"default":"properties-for-sale-in-ayia-napa",ru:"prodazha-nedvizhimosti-v-ayia-nape"},FAMAGUSTA_SALE:{"default":"properties-for-sale-in-famagusta",ru:"prodazha-nedvizhimosti-v-famaguste"},LARNACA_SALE:{"default":"properties-for-sale-in-larnaca",ru:"prodazha-nedvizhimosti-v-larnake"},LIMASSOL_SALE:{"default":"properties-for-sale-in-limassol",ru:"prodazha-nedvizhimosti-v-limassole"},NICOSIA_SALE:{"default":"properties-for-sale-in-nicosia",ru:"prodazha-nedvizhimosti-v-nikosii"},PAPHOS_SALE:{"default":"properties-for-sale-in-paphos",ru:"prodazha-nedvizhimosti-v-pafose"},PROTARAS_SALE:{"default":"properties-for-sale-in-protaras",ru:"prodazha-nedvizhimosti-v-protarase"},TROODOS_SALE:{"default":"properties-for-sale-in-troodos",ru:"prodazha-nedvizhimosti-v-troodose"},APARTMENT:{"default":"apartments-in-cyprus",ru:"apartamenti-na-kipre"},AYIA_NAPA_APARTMENT:{"default":"apartments-in-ayia-napa",ru:"apartamenti-v-ayia-nape"},FAMAGUSTA_APARTMENT:{"default":"apartments-in-famagusta",ru:"apartamenti-v-famaguste"},LARNACA_APARTMENT:{"default":"apartments-in-larnaca",ru:"apartamenti-v-larnake"},LIMASSOL_APARTMENT:{"default":"apartments-in-limassol",ru:"apartamenti-v-limassole"},NICOSIA_APARTMENT:{"default":"apartments-in-nicosia",ru:"apartamenti-v-nikosii"},PAPHOS_APARTMENT:{"default":"apartments-in-paphos",ru:"apartamenti-v-pafose"},PROTARAS_APARTMENT:{"default":"apartments-in-protaras",ru:"apartamenti-v-protarase"},TROODOS_APARTMENT:{"default":"apartments-in-troodos",ru:"apartamenti-v-troodose"},LONG_TERM_APARTMENT:{"default":"apartments-long-term-rental-in-cyprus",ru:"arenda-apartamentov-na-kipre"},AYIA_NAPA_LONG_TERM_APARTMENT:{"default":"apartments-long-term-rental-in-ayia-napa",ru:"arenda-apartamentov-v-ayia-nape"},FAMAGUSTA_LONG_TERM_APARTMENT:{"default":"apartments-long-term-rental-in-famagusta",ru:"arenda-apartamentov-v-famaguste"},LARNACA_LONG_TERM_APARTMENT:{"default":"apartments-long-term-rental-in-larnaca",ru:"arenda-apartamentov-v-larnake"},LIMASSOL_LONG_TERM_APARTMENT:{"default":"apartments-long-term-rental-in-limassol",ru:"arenda-apartamentov-v-limassole"},NICOSIA_LONG_TERM_APARTMENT:{"default":"apartments-long-term-rental-in-nicosia",ru:"arenda-apartamentov-v-nikosii"},PAPHOS_LONG_TERM_APARTMENT:{"default":"apartments-long-term-rental-in-paphos",ru:"arenda-apartamentov-v-pafose"},PROTARAS_LONG_TERM_APARTMENT:{"default":"apartments-long-term-rental-in-protaras",ru:"arenda-apartamentov-v-protarase"},TROODOS_LONG_TERM_APARTMENT:{"default":"apartments-long-term-rental-in-troodos",ru:"arenda-apartamentov-v-troodose"},SHORT_TERM_APARTMENT:{"default":"apartments-short-term-rental-in-cyprus",ru:"otdih-na-kipre-apartamenti"},AYIA_NAPA_SHORT_TERM_APARTMENT:{"default":"apartments-short-term-rental-in-ayia-napa",ru:"otdih-na-kipre-v-ayia-nape-apartamenti"},FAMAGUSTA_SHORT_TERM_APARTMENT:{"default":"apartments-short-term-rental-in-famagusta",ru:"otdih-na-kipre-v-famaguste-apartamenti"},LARNACA_SHORT_TERM_APARTMENT:{"default":"apartments-short-term-rental-in-larnaca",ru:"otdih-na-kipre-v-larnake-apartamenti"},LIMASSOL_SHORT_TERM_APARTMENT:{"default":"apartments-short-term-rental-in-limassol",ru:"otdih-na-kipre-v-limassole-apartamenti"},NICOSIA_SHORT_TERM_APARTMENT:{"default":"apartments-short-term-rental-in-nicosia",ru:"otdih-na-kipre-v-nikosii-apartamenti"},PAPHOS_SHORT_TERM_APARTMENT:{"default":"apartments-short-term-rental-in-paphos",ru:"otdih-na-kipre-v-pafose-apartamenti"},PROTARAS_SHORT_TERM_APARTMENT:{"default":"apartments-short-term-rental-in-protaras",ru:"otdih-na-kipre-v-protarase-apartamenti"},TROODOS_SHORT_TERM_APARTMENT:{"default":"apartments-short-term-rental-in-troodos",ru:"otdih-na-kipre-v-troodose-apartamenti"},SALE_APARTMENT:{"default":"apartments-for-sale-in-cyprus",ru:"prodazha-apartamentov-na-kipre"},AYIA_NAPA_SALE_APARTMENT:{"default":"apartments-for-sale-in-ayia-napa",ru:"prodazha-apartamentov-v-ayia-nape"},FAMAGUSTA_SALE_APARTMENT:{"default":"apartments-for-sale-in-famagusta",ru:"prodazha-apartamentov-v-famaguste"},LARNACA_SALE_APARTMENT:{"default":"apartments-for-sale-in-larnaca",ru:"prodazha-apartamentov-v-larnake"},LIMASSOL_SALE_APARTMENT:{"default":"apartments-for-sale-in-limassol",ru:"prodazha-apartamentov-v-limassole"},NICOSIA_SALE_APARTMENT:{"default":"apartments-for-sale-in-nicosia",ru:"prodazha-apartamentov-v-nikosii"},PAPHOS_SALE_APARTMENT:{"default":"apartments-for-sale-in-paphos",ru:"prodazha-apartamentov-v-pafose"},PROTARAS_SALE_APARTMENT:{"default":"apartments-for-sale-in-protaras",ru:"prodazha-apartamentov-v-protarase"},TROODOS_SALE_APARTMENT:{"default":"apartments-for-sale-in-troodos",ru:"prodazha-apartamentov-v-troodose"},HOUSE:{"default":"houses-in-cyprus",ru:"doma-na-kipre"},AYIA_NAPA_HOUSE:{"default":"houses-in-ayia-napa",ru:"doma-v-ayia-nape"},FAMAGUSTA_HOUSE:{"default":"houses-in-famagusta",ru:"doma-v-famaguste"},LARNACA_HOUSE:{"default":"houses-in-larnaca",ru:"doma-v-larnake"},LIMASSOL_HOUSE:{"default":"houses-in-limassol",ru:"doma-v-limassole"},NICOSIA_HOUSE:{"default":"houses-in-nicosia",ru:"doma-v-nikosii"},PAPHOS_HOUSE:{"default":"houses-in-paphos",ru:"doma-v-pafose"},PROTARAS_HOUSE:{"default":"houses-in-protaras",ru:"doma-v-protarase"},TROODOS_HOUSE:{"default":"houses-in-troodos",ru:"doma-v-troodose"},LONG_TERM_HOUSE:{"default":"houses-long-term-rental-in-cyprus",ru:"arenda-domov-na-kipre"},AYIA_NAPA_LONG_TERM_HOUSE:{"default":"houses-long-term-rental-in-ayia-napa",ru:"arenda-domov-v-ayia-nape"},FAMAGUSTA_LONG_TERM_HOUSE:{"default":"houses-long-term-rental-in-famagusta",ru:"arenda-domov-v-famaguste"},LARNACA_LONG_TERM_HOUSE:{"default":"houses-long-term-rental-in-larnaca",ru:"arenda-domov-v-larnake"},LIMASSOL_LONG_TERM_HOUSE:{"default":"houses-long-term-rental-in-limassol",ru:"arenda-domov-v-limassole"},NICOSIA_LONG_TERM_HOUSE:{"default":"houses-long-term-rental-in-nicosia",ru:"arenda-domov-v-nikosii"},PAPHOS_LONG_TERM_HOUSE:{"default":"houses-long-term-rental-in-paphos",ru:"arenda-domov-v-pafose"},PROTARAS_LONG_TERM_HOUSE:{"default":"houses-long-term-rental-in-protaras",ru:"arenda-domov-v-protarase"},TROODOS_LONG_TERM_HOUSE:{"default":"houses-long-term-rental-in-troodos",ru:"arenda-domov-v-troodose"},SHORT_TERM_HOUSE:{"default":"houses-short-term-rental-in-cyprus",ru:"otdih-na-kipre-doma"},AYIA_NAPA_SHORT_TERM_HOUSE:{"default":"houses-short-term-rental-in-ayia-napa",ru:"otdih-na-kipre-v-ayia-nape-doma"},FAMAGUSTA_SHORT_TERM_HOUSE:{"default":"houses-short-term-rental-in-famagusta",ru:"otdih-na-kipre-v-famaguste-doma"},LARNACA_SHORT_TERM_HOUSE:{"default":"houses-short-term-rental-in-larnaca",ru:"otdih-na-kipre-v-larnake-doma"},LIMASSOL_SHORT_TERM_HOUSE:{"default":"houses-short-term-rental-in-limassol",ru:"otdih-na-kipre-v-limassole-doma"},NICOSIA_SHORT_TERM_HOUSE:{"default":"houses-short-term-rental-in-nicosia",ru:"otdih-na-kipre-v-nikosii-doma"},PAPHOS_SHORT_TERM_HOUSE:{"default":"houses-short-term-rental-in-paphos",ru:"otdih-na-kipre-v-pafose-doma"},PROTARAS_SHORT_TERM_HOUSE:{"default":"houses-short-term-rental-in-protaras",ru:"otdih-na-kipre-v-protarase-doma"},TROODOS_SHORT_TERM_HOUSE:{"default":"houses-short-term-rental-in-troodos",ru:"otdih-na-kipre-v-troodose-doma"},SALE_HOUSE:{"default":"houses-for-sale-in-cyprus",ru:"prodazha-domov-na-kipre"},AYIA_NAPA_SALE_HOUSE:{"default":"houses-for-sale-in-ayia-napa",ru:"prodazha-domov-v-ayia-nape"},FAMAGUSTA_SALE_HOUSE:{"default":"houses-for-sale-in-famagusta",ru:"prodazha-domov-v-famaguste"},LARNACA_SALE_HOUSE:{"default":"houses-for-sale-in-larnaca",ru:"prodazha-domov-v-larnake"},LIMASSOL_SALE_HOUSE:{"default":"houses-for-sale-in-limassol",ru:"prodazha-domov-v-limassole"},NICOSIA_SALE_HOUSE:{"default":"houses-for-sale-in-nicosia",ru:"prodazha-domov-v-nikosii"},PAPHOS_SALE_HOUSE:{"default":"houses-for-sale-in-paphos",ru:"prodazha-domov-v-pafose"},PROTARAS_SALE_HOUSE:{"default":"houses-for-sale-in-protaras",ru:"prodazha-domov-v-protarase"},TROODOS_SALE_HOUSE:{"default":"houses-for-sale-in-troodos",ru:"prodazha-domov-v-troodose"},LAND:{"default":"land-in-cyprus",ru:"zemlya-na-kipre"},AYIA_NAPA_LAND:{"default":"land-in-ayia-napa",ru:"zemlya-v-ayia-nape"},FAMAGUSTA_LAND:{"default":"land-in-famagusta",ru:"zemlya-v-famaguste"},LARNACA_LAND:{"default":"land-in-larnaca",ru:"zemlya-v-larnake"},LIMASSOL_LAND:{"default":"land-in-limassol",ru:"zemlya-v-limassole"},NICOSIA_LAND:{"default":"land-in-nicosia",ru:"zemlya-v-nikosii"},PAPHOS_LAND:{"default":"land-in-paphos",ru:"zemlya-v-pafose"},PROTARAS_LAND:{"default":"land-in-protaras",ru:"zemlya-v-protarase"},TROODOS_LAND:{"default":"land-in-troodos",ru:"zemlya-v-troodose"},LONG_TERM_LAND:{"default":"land-long-term-rental-in-cyprus",ru:"arenda-zemli-na-kipre"},AYIA_NAPA_LONG_TERM_LAND:{"default":"land-long-term-rental-in-ayia-napa",ru:"arenda-zemli-v-ayia-nape"},FAMAGUSTA_LONG_TERM_LAND:{"default":"land-long-term-rental-in-famagusta",ru:"arenda-zemli-v-famaguste"},LARNACA_LONG_TERM_LAND:{"default":"land-long-term-rental-in-larnaca",ru:"arenda-zemli-v-larnake"},LIMASSOL_LONG_TERM_LAND:{"default":"land-long-term-rental-in-limassol",ru:"arenda-zemli-v-limassole"},NICOSIA_LONG_TERM_LAND:{"default":"land-long-term-rental-in-nicosia",ru:"arenda-zemli-v-nikosii"},PAPHOS_LONG_TERM_LAND:{"default":"land-long-term-rental-in-paphos",ru:"arenda-zemli-v-pafose"},PROTARAS_LONG_TERM_LAND:{"default":"land-long-term-rental-in-protaras",ru:"arenda-zemli-v-protarase"},TROODOS_LONG_TERM_LAND:{"default":"land-long-term-rental-in-troodos",ru:"arenda-zemli-v-troodose"},SHORT_TERM_LAND:{"default":"land-short-term-rental-in-cyprus",ru:"otdih-na-kipre-zemlya"},AYIA_NAPA_SHORT_TERM_LAND:{"default":"land-short-term-rental-in-ayia-napa",ru:"otdih-na-kipre-v-ayia-nape-zemlya"},FAMAGUSTA_SHORT_TERM_LAND:{"default":"land-short-term-rental-in-famagusta",ru:"otdih-na-kipre-v-famaguste-zemlya"},LARNACA_SHORT_TERM_LAND:{"default":"land-short-term-rental-in-larnaca",ru:"otdih-na-kipre-v-larnake-zemlya"},LIMASSOL_SHORT_TERM_LAND:{"default":"land-short-term-rental-in-limassol",ru:"otdih-na-kipre-v-limassole-zemlya"},NICOSIA_SHORT_TERM_LAND:{"default":"land-short-term-rental-in-nicosia",ru:"otdih-na-kipre-v-nikosii-zemlya"},PAPHOS_SHORT_TERM_LAND:{"default":"land-short-term-rental-in-paphos",ru:"otdih-na-kipre-v-pafose-zemlya"},PROTARAS_SHORT_TERM_LAND:{"default":"land-short-term-rental-in-protaras",ru:"otdih-na-kipre-v-protarase-zemlya"},TROODOS_SHORT_TERM_LAND:{"default":"land-short-term-rental-in-troodos",ru:"otdih-na-kipre-v-troodose-zemlya"},SALE_LAND:{"default":"land-for-sale-in-cyprus",ru:"prodazha-zemli-na-kipre"},AYIA_NAPA_SALE_LAND:{"default":"land-for-sale-in-ayia-napa",ru:"prodazha-zemli-v-ayia-nape"},FAMAGUSTA_SALE_LAND:{"default":"land-for-sale-in-famagusta",ru:"prodazha-zemli-v-famaguste"},LARNACA_SALE_LAND:{"default":"land-for-sale-in-larnaca",ru:"prodazha-zemli-v-larnake"},LIMASSOL_SALE_LAND:{"default":"land-for-sale-in-limassol",ru:"prodazha-zemli-v-limassole"},NICOSIA_SALE_LAND:{"default":"land-for-sale-in-nicosia",ru:"prodazha-zemli-v-nikosii"},PAPHOS_SALE_LAND:{"default":"land-for-sale-in-paphos",ru:"prodazha-zemli-v-pafose"},PROTARAS_SALE_LAND:{"default":"land-for-sale-in-protaras",ru:"prodazha-zemli-v-protarase"},TROODOS_SALE_LAND:{"default":"land-for-sale-in-troodos",ru:"prodazha-zemli-v-troodose"}};if(typeof(Utils)==="undefined"){throw"Utils class must be defined. Include utils.js file before this one."}Utils.getPath=function(a){return"/parent/webapp/public"+a};var HomeIndexPage={};HomeIndexPage.init=function(c,b,a){HomeIndexPage.staticMapCreated=false;HomeIndexPage.dynamicMapCreated=false;HomeIndexPage.dynamicMapHeight=700;HomeIndexPage.lat=function(){return b};HomeIndexPage.lng=function(){return a};HomeIndexPage.manageMaps();$(window).on("resize",HomeIndexPage.manageMaps);$("#properties-search-form").submit(function(){var i=$("#location");var e=$("#location").val();i.prop("disabled",true);var h=$("#offerType");var d=$("#offerType").val();h.prop("disabled",true);var g=$("#propertyType");var f=$("#propertyType").val();g.prop("disabled",true);$(this).attr("action",Utils.getPropertySearchPath(c,e,d,f))})};HomeIndexPage.manageMaps=function(){if(HomeIndexPage.dynamicMapCreated){return}var a=$(window).width();if(a>=768){if(a<992){if(!HomeIndexPage.staticMapCreated){HomeIndexPage.createStatic();HomeIndexPage.centerSearchBox()}}else{HomeIndexPage.createDynamic();HomeIndexPage.centerSearchBox()}}};HomeIndexPage.createDynamic=function(){HomeIndexPage.dynamicMapCreated=true;$("#map").css("height",HomeIndexPage.dynamicMapHeight+"px");Utils.appendScript("http://maps.google.com/maps/api/js?sensor=false&callback=HomeIndexPage.createHomepageGoogleMap")};HomeIndexPage.createHomepageGoogleMap=function(){Utils.appendScript(Utils.getPath("/assets/js/application-maps.min.js"));$.getJSON("properties.json",function(f){var a=new google.maps.Map(document.getElementById("map"),{zoom:9,scrollwheel:false,zoomControl:true,zoomControlOptions:{position:google.maps.ControlPosition.LEFT_TOP},streetViewControl:false,center:new google.maps.LatLng(HomeIndexPage.lat(),HomeIndexPage.lng()),mapTypeId:google.maps.MapTypeId.ROADMAP,styles:mapStyles});var j={disableAutoPan:false,boxStyle:{width:"250px"},pixelOffset:new google.maps.Size(-100,0),zIndex:null,alignBottom:true,boxClass:"infobox-wrapper",enableEventPropagation:true,closeBoxMargin:"0px 0px -8px 0px",closeBoxURL:"http://assets.gotocy.com/static/img/close-btn.png",infoBoxClearance:new google.maps.Size(1,1)};var c=[];for(var d=0;d<f.length;d++){var g=document.createElement("img");g.src="http://assets.gotocy.com/static/img/property-types/"+f[d]["typeIcon"]+".png";var e=new MarkerWithLabel({title:f[d]["title"],position:new google.maps.LatLng(f[d]["latitude"],f[d]["longitude"]),icon:"http://assets.gotocy.com/static/img/marker.png",labelContent:g,labelAnchor:new google.maps.Point(50,0),labelClass:"marker-style"});c.push(e);google.maps.event.addListener(e,"click",(function(k){return function(){if(!c[k].infobox){var l=document.createElement("div");$(l).html('<div class="infobox-inner"><a href="'+f[k]["propertyUrl"]+'"><div class="infobox-image" style="position: relative"><img src="'+f[k]["representativeImageUrl"]+'"><div><span class="infobox-price">'+f[k]["price"]+'</span></div></div></a><div class="infobox-description"><div class="infobox-title"><a href="'+f[k]["propertyUrl"]+'">'+f[k]["title"]+'</a></div><div class="infobox-location">'+f[k]["shortAddress"]+"</div></div></div>");c[k].infobox=new InfoBox($.extend({content:l},j))}for(var i=0;i<c.length;i++){if(c[i].infobox){c[i].infobox.close()}}c[k].infobox.open(a,this)}})(d))}var b=[{url:"http://assets.gotocy.com/static/img/cluster.png",height:37,width:37}];var h=new MarkerClusterer(a,c,{styles:b,maxZoom:15});$("body").addClass("loaded");setTimeout(function(){$("body").removeClass("has-fullscreen-map")},1000);$("#map").removeClass("fade-map")})};HomeIndexPage.createStatic=function(){staticMapCreated=true;var b=$(window).width()/2.13;$("#map").css("height",b);var a=$(document.createElement("img"));a.attr("src","http://maps.googleapis.com/maps/api/staticmap?center="+HomeIndexPage.lat()+","+HomeIndexPage.lng()+"&zoom=8&size=768x300&scale=2&style=feature:water|element:all|hue:0xd7ebef|saturation:-5|lightness:54|visibility:on&style=feature:landscape|element:all|hue:0xeceae6|saturation:-49|lightness:22|visibility:on&style=feature:poi.ark|element:all|hue:0xdddbd7|saturation:-81|lightness:34|visibility:on&style=feature:poi.medical|element:all|hue:0xdddbd7|saturation:-80|lightness:-2|visibility:on&style=feature:poi.school|element:all|hue:0xc8c6c3|saturation:-91|lightness:-7|visibility:on&style=feature:landscape.natural|element:all|hue:0xc8c6c3|saturation:-71|lightness:-18|visibility:on&style=feature:road.highway|element:all|hue:0xdddbd7|saturation:-92|lightness:60|visibility:on&style=feature:poi|element:all|hue:0xdddbd7|saturation:-81|lightness:34|visibility:on&style=feature:road.arterial|element:all|hue:0xdddbd7|saturation:-81|lightness:34|visibility:on&style=feature:transit|element:geometry|hue:0xc8c6c3|saturation:4|lightness:10|visibility:on&key=AIzaSyDHd0r7iRvEzgEwPscSKbRmU6eIoL8LcUs");
a.addClass("img-responsive");a.addClass("center");a.css("height",b);$("#map").html(a)};HomeIndexPage.centerSearchBox=function(){var c=$("#map").height();var a=$(".navigation").height();var b=$(".search-box").height();var d=c===HomeIndexPage.dynamicMapHeight?c+a-b*2:c*0.7+a;$(".search-box-wrapper").css("top",d)};var PropertyIndexPage={};PropertyIndexPage.init=function(a){$("#form-sidebar").submit(function(){var g=$("#location");var c=g.val();g.prop("disabled",true);var f=$("#offerType");var b=f.val();f.prop("disabled",true);var e=$("#propertyType");var d=e.val();e.prop("disabled",true);$(this).attr("action",Utils.getPropertySearchPath(a,c,b,d))})};var PropertySubmissionForm={};PropertySubmissionForm.init=function(d,b,c,a){if(c!=="en"){Utils.appendScript(Utils.getPath("/assets/js/fileinput_locale_"+c+".js"))}$("#images-upload").fileinput({language:c,maxFileCount:d,maxFileSize:b,showUpload:false,showCaption:false,showRemove:false,showClose:false,browseClass:"btn btn-default",browseLabel:a})};ImagesEditor={};ImagesEditor.init=function(){var b=$(".image-select");if(b.length>0){ImagesEditor.withSelect=true;$(".image-select button").click(function(c){$(this).toggleClass("btn-success");$(this).parent(".image-select").parent(".image").toggleClass("selected");c.preventDefault()})}var a=$(".image-remove");if(a.length>0){$(".image-remove a").click(function(h){var g=$(this).parent(".image-remove").parent(".image");g.toggleClass("removed");g.removeClass("selected");if(ImagesEditor.withSelect){g.children(".image-select").children("button").removeClass("btn-success")}var f=g.attr("id");var d=f.substring(6,f.length-1);var c=g.children("#imagesState"+d+"\\.removed");c.val(c.val()=="false"?"true":"false");h.preventDefault()})}$("#images-editor").magnificPopup({delegate:"a.image-popup",type:"image",gallery:{enabled:true,navigateByImgClick:true,preload:[0,1]},overflowY:"scroll"})};